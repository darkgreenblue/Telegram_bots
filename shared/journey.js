...
/* 📤 ثبتِ پیامِ **بدونِ ctx** — پیامی که ربات خودش می‌فرستد، نه در جوابِ یک آپدیت.
 *
 * چرا لازم شد (مشاهده‌ی واقعیِ مالک ۱۴۰۵/۰۶/۱۵): پیام‌های صفِ ادمین (تأییدِ رسید،
 * شارژِ پشتیبانی) با `bot.telegram.sendMessage` می‌روند و هیچ `ctx` ای ندارند، پس
 * میدل‌ور نمی‌تواند رپشان کند و در «بازپخشِ مسیر» **نامرئی** بودند. مالک دید که کاربر
 * الماسش را گرفته ولی هیچ پیامی در تایم‌لاین نیست و فکر کرد پیام نرفته — در حالی که
 * رفته بود. صفحه‌ای که ادعا می‌کند کلِ مسیرِ کاربر را نشان می‌دهد، نباید دقیقاً
 * پیام‌های **مالی** را جا بیندازد.
 *
 * ⚠️ عمداً یک تابعِ **صریح** است نه رپِ خودکارِ `telegram.sendMessage`: رپِ خودکار
 * انیمیشن‌ها و پیام‌های فنی (که همین حالا هم عمداً بیرونِ قیف‌اند) را هم می‌گرفت و
 * دقیقاً همان آلودگی‌ای را می‌ساخت که قرارداد از آن پرهیز می‌کند. هر صداکننده خودش
 * تصمیم می‌گیرد که این پیام بخشی از مسیرِ کاربر هست یا نه.
 *
 * پراپِ `p:1` می‌گوید این یک پیامِ فرستاده‌شده از سمتِ سرور است، تا در تحلیل بشود
 * جدایش کرد از صفحه‌ای که کاربر با اقدامِ خودش رسیده.
 * fail-safe: هر خطا فقط لاگ می‌شود و هرگز مسیرِ پول را نمی‌شکند. */
export function logPush(db, userId, text, { isAdmin = false, label = '' } = {}) {
  try {
    if (!db || !userId) return;
    const raw = String(text ?? '');
    const k = label ? String(label).slice(0, 40)
      : (raw.length > CONTENT_MIN_CHARS ? 'content' : hash8('|' + normText(raw, [])));
    const props = { k, t: 'push', p: 1 };
    if (isAdmin) props.adm = 1;
    if (k !== 'content') catalog(db, k, { label, kind: 'push', sample: raw, buttons: '' });
    track(db, userId, JOURNEY_EVENTS.VIEW, props);
  } catch (e) { logErr('journey push:', e.message); }
}
