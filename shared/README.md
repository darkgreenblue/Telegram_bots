...
| `journey.js` | `registerJourney, ensureJourney, JOURNEY_SCHEMA_VERSION` | **مسیرِ ریزِ کاربر**: میدل‌ورِ Telegraf که خودکار هر پیامِ خروجی (`view`) و هر اکشنِ ورودی (`act`) را ثبت می‌کند + جدولِ کاتالوگِ `screens` (کلیدِ صفحه → متنِ واقعیِ پیام). بدونِ برچسب‌گذاریِ دستی؛ باید **قبل از همه‌ی هندلرها** ثبت شود. متنِ کاربر هرگز ذخیره نمی‌شود. چکِ CI: `tools/check-journey.mjs` |
| `initdata.js` | `verifyInitData, initDataUserId, INITDATA_MAX_AGE_SEC` | تأییدِ امضای `initData` مینی‌اپِ تلگرام (HMAC-SHA256 با کلیدِ `WebAppData`) + سقفِ سنی. **تنها دروازه‌ی هویت در مسیرِ وب**: هیچ مسیری حق ندارد `user_id` را از بدنه‌ی درخواست باور کند. هرگز throw نمی‌کند. چکِ CI: `tools/check-initdata.mjs` (۳۵ ادعا که بیشترشان منفی‌اند). طراحی و وضعیت: `bots/tarot/MINIAPP.md` |
