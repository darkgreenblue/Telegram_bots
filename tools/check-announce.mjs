...
console.log('▶ قواعدِ کپیِ سراسری');
{
  ...
  ok(CTA === 'کارت شانس امروزت رو از دست نده و فرصت کمی براش مونده.', 'متنِ CTA دقیقاً همان چیزی است که مالک نوشت');
}
...
console.log('\n▶ ساختارِ سه نسخه');
{
  ...
  ok(/موجودی ذخایر الماس: 7 الماس/.test(a), 'نسخه‌ی A عددِ الماس را می‌گوید');
  ok(/موجودی ذخایر الماس: 5 الماس/.test(b), 'نسخه‌ی B هم عددِ الماس را می‌گوید');
  ...
  ok(b.split('\n').filter(Boolean).at(-2) === 'موجودی ذخایر الماس: 5 الماس',
