...
/* و همین ادعا روی کلِ سورس: هیچ‌جای **ناخواسته‌ای** نباید حاملِ دومی بسازد.
 * ⚠️ از v3.95.0 حامل **دو** مصرف دارد و این عمدی است، پس ادعا از «یک» به «دقیقاً دو
 * تا، و دقیقاً همین دو» تیزتر شد (نه خفه‌تر): `ensureKeyboard` تورِ خودکار است و
 * `deliverKeyboard` تحویلِ صریحی که فقط با اقدامِ خودِ کاربر صدا زده می‌شود. هر مصرفِ
 * سوم یعنی یک حاملِ ناخواسته و قرمز می‌دهد. */
ok((SRC.match(/kbRefresh/g) || []).length === 2, 'متنِ حامل دقیقاً دو مصرفِ اعلام‌شده دارد');
const dk = bodyOf('async function deliverKeyboard(tg, uid) {');
ok(!!dk, 'تحویلِ صریحِ کیبورد (deliverKeyboard) پیدا شد');
ok(dk ? /kbRefresh/.test(dk) : false, 'و مصرفِ دومِ حامل دقیقاً همان‌جاست');
ok(dk ? /tg\.sendMessage\(/.test(dk) && /tg\.deleteMessage\(/.test(dk) : false,
  'تحویلِ صریح هم از tg می‌رود و حاملش را حذف می‌کند');
ok(dk ? /disable_notification: true/.test(dk) : false, 'و بی‌صداست (پیامِ دیده‌شدنی اضافه نمی‌کند)');
ok(dk ? !/ctx\./.test(dk) : false, 'و هرگز به ctx دست نمی‌زند (صفر رویدادِ جعلیِ جرنی)');
/* 🔑 مهر **بعد از** ارسالِ تأییدشده (درسِ v3.93.0). اگر قبلش بزند، `kb_shown_at` درباره‌ی
 * کیبوردی که هرگز نرسیده دروغ می‌گوید و تورِ ترمیم برای آن کاربر کور می‌شود. */
ok(dk ? dk.indexOf('setKbShown') > dk.indexOf('tg.sendMessage(') : false,
  '🔑 و مهرِ کیبورد بعد از ارسالِ موفق می‌خورد، نه قبلش');
/* ⚠️ و تحویلِ صریح **گاردِ استیتِ ساکت ندارد** و این هم عمدی است: تنها صداکننده‌هایش
 * دکمه‌ی «پایان مکالمه» و دستورِ `/menu` اند، یعنی جایی که خودِ کاربر منو را خواسته. */
ok(dk ? !/KB_QUIET_STATES/.test(dk) : false,
  'و عمداً پشتِ گاردِ استیتِ ساکت نیست (اقدامِ صریحِ کاربر است، نه تورِ خودکار)');
