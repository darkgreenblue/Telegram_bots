...
const TARGETS = [
  ...
  'tools/chat-lab.mjs',
  'tools/reading-lab/chat-checks.mjs',
  'tools/reading-lab/chat-rubric.mjs',
...
//
// ⚠️ حدِ صداقتِ این تابع، که یک بار قرمزِ کاذب داد (۱۴۰۵/۰۶/۲۷، `tools/reading-lab.mjs`):
// یک template literal که هم `${...}` دارد و هم یک اسلشِ برهنه بینِ دو درج — مثلِ
// شکلِ «دلار-آکولاد lo آکولاد، اسلش، دلار-آکولاد to آکولاد» — درست تشخیص داده
// نمی‌شود؛ آن اسلش به‌عنوانِ شروعِ regex خوانده می‌شود و وضعیتِ پارسر به هم می‌ریزد.
// عارضه‌اش این است که **کامنتِ بلوکیِ بعدِ همان خط هم استریپ نمی‌شود**، پس هر نامِ
// تابعی که داخلِ آن کامنت نوشته شود به‌عنوانِ شناسه‌ی تعریف‌نشده گزارش می‌شود.
// تا وقتی این رفع نشده، قاعده‌ی عملی: در کامنت‌های همان ناحیه نامِ تابع را با
// پرانتز ننویس. (پنجمین بارِ ثبت‌شده‌ی خانواده‌ی «کامنت، خودش باگ تولید می‌کند».)
