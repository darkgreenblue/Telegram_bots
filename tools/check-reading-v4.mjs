...
console.log('\n▶ چهار حرکتِ وزن‌بالا (هم در تحقیق‌ها، هم در خوانشِ واقعی)');
{
  ...
  // کارتِ نیامده از **کد** انتخاب می‌شود، نه مدل: نسخه‌ی آزادِ قبلی در ۷ فال از ۷ یک
  // قالبِ واحدِ مبهم داد («خوشبختانه کارت‌های سنگین نیومدن») که غیرقابلِ ابطال است.
  // خوانشِ انسانیِ واقعی کارت را **نام می‌برد** («کارت فروپاشی نیفتاد کلا»).
  ok(/کارتِ سنگینی که نیامده/.test(V4) && /\*\*با نامِ خودش\*\*/.test(V4),
    'فضای خالی: کارتِ نیامده با نامِ خودش گفته می‌شود');
  ok(/جمله‌ی کلی مثل «کارتِ سنگینی نیامده» ننویس/.test(V4), 'جمله‌ی کلیِ بی‌نام صریحاً ممنوع است');
  ok(/export function absentHeavy/.test(CORE), 'انتخابِ کارتِ نیامده در کد است');
  ok(/'کارتِ سنگینی که نیامده': absentHeavy\(cards\)/.test(CORE), 'به کانتکست تزریق می‌شود');
  {
    const a = core.absentHeavy([{ key: 'c06' }, { key: 'c04' }, { key: 'm00' }]);
    ok(a && a.fa && a.means, 'کارتِ سنگینِ نیامده انتخاب می‌شود');
    ok(JSON.stringify(core.absentHeavy([{ key: 'c06' }, { key: 'c04' }, { key: 'm00' }])) === JSON.stringify(a),
      'انتخاب قطعی است (بعد از ری‌استارت عوض نمی‌شود)');
    const drawn = core.absentHeavy([{ key: 'm16' }, { key: 'm13' }, { key: 'm15' }]);
    ok(drawn && !['برج', 'مرگ', 'شیطان'].includes(drawn.fa), 'کارتِ کشیده‌شده هرگز به‌عنوان «نیامده» انتخاب نمی‌شود');
  }
}
...
console.log('\n▶ درس‌های دورِ سوم (۱۴۰۵/۰۵/۲۶ عصر)');
{
  ...
  ok(/ادعای زیاد نپاشان به امیدِ اینکه یکی بچسبد/.test(V4), 'شات‌گانینگ صریحاً ممنوع است');
  ok(/بشود عیناً برای کاربرِ دیگری فرستاد، شکست‌خورده/.test(V4), 'متنِ عمومی = شکست');
