...
console.log('\n▶ درسِ اولین اجرای آزمایشگاه (۱۴۰۵/۰۵/۲۶)');
{
  ...
  // زمانِ ساختگی از فیلدِ callback به بقیه‌ی متن سرریز کرد («هفته‌های قبل» برای فالی
  // که ۴۰ دقیقه قبل بود)، پس قاعده باید سراسری باشد نه per فیلد.
  ok(/\*\*زمان را از خودت نساز، در هیچ بخشی\.\*\*/.test(V4), 'قاعده‌ی زمان سراسری شد، نه فقط داخلِ callback');
}
...
console.log('\n▶ درس‌های دورِ سوم (۱۴۰۵/۰۵/۲۶ عصر)');
{
  ...
  ok(/export function agoFa\(unixSec/.test(CORE), 'محاسبه‌ی فاصله در کد است، نه حدسِ مدل');
  {
    ...
    ok(agoFa(now - 86400 * 400) === '1 سال پیش', 'یک سال پیش درست گفته می‌شود');
    ok(agoFa(now - 600) !== 'پارسال', 'فالِ ۱۰ دقیقه‌ای هرگز «پارسال» نمی‌شود');
  }
  ok(/«پارسال» و هر تخمینِ دیگری/.test(V4), 'پرامپت هم ساختنِ زمان را ممنوع می‌کند');
