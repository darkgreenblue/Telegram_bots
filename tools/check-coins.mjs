...
console.log('\n▶ قواعدِ کپیِ پرامپتِ جدید (منبع: bots/tarot/STYLE.md)');
{
  ...
  ok(p.length > 1500, 'پرامپتِ نسخه‌ی دوم پیدا شد');

  // «احتمال آری، طفره نه» — مهم‌ترین تمایزی که نسخه‌ی اولِ پرامپت اشتباه گرفت: تاروت‌خوانِ
  // واقعی آزادانه «احتمالا/ممکنه» می‌گوید ولی همیشه با یک جهت. بنابراین این کلمه‌ها نباید
  // یک‌جا ممنوع شوند؛ چیزی که ممنوع است جوابِ بی‌جهت است.
  ...
  ok(/باید یک جهت داشته باشد/.test(p), 'قاعده‌ی «هر جمله‌ی احتمالی باید جهت داشته باشد» هست');

  // بازخوردِ صریحِ مالک: «نشونه» یک مفهوم است، نه برچسبی که ربات اسمش را ببرد
  ok(/هرگز ننویس «نشونه‌ات اینه»/.test(p), 'برچسبِ «نشونه‌ات» صریحاً ممنوع شده');
  ok(!/🔎/.test(p), 'هیچ برچسبِ ایموجی‌داری در پرامپت نمانده');

  // صدا
  ok(/فارسیِ گفتاری/.test(p), 'فارسیِ گفتاری (نه کتابی، نه لحنِ دستیار) خواسته شده');
  ok(/تراپیست نیستی/.test(p), 'لحنِ تراپیستی/لوس‌کننده ممنوع شده');
  ok(/بگو چه چیزی نیامده/.test(p), 'حرکتِ «بگو چه کارتی نیامده» در پرامپت هست');
  ok(/کارتِ درباری/.test(p), 'حرکتِ «کارتِ درباری را به آدمِ واقعی ترجمه کن» هست');
  ok(/نمونه‌ی یک/.test(p) && /نمونه‌ی دو/.test(p), 'دو نمونه‌ی واقعی برای تقلیدِ صدا در پرامپت هست');

  ok(/کلمه‌های «سرگرمی»، «فان»/.test(p), 'کلمه‌ی «سرگرمی»/«فان» در خروجی ممنوع شده');
  ok(/هیچ هشداری درباره‌ی اینکه این خوانش نباید مبنای تصمیم باشد/.test(p), 'هشدارِ «مبنای تصمیم نباشه» حذف شده');
  ok(/هیچ شعارِ پایانی/.test(p), 'شعارِ پایانی ممنوع شده');
  ...
  // رندرِ خروجی: نسخه‌ی جدید بدونِ برچسب، نسخه‌ی قدیم دست‌نخورده (کاربرِ واقعی روی آن است)
  ...
  // شعارِ پایانی فقط برای لحنِ قدیم فرستاده می‌شود
  ok(/if \(!toneV2For\(uid\)\) \{[\s\S]{0,200}empowerClose/.test(SRC),
    'شعارِ پایانی در لحنِ جدید اصلاً فرستاده نمی‌شود');
}
