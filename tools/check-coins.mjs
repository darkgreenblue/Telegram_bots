...
console.log('\n▶ نامِ «جای موجودی»: کیف‌پول در دنیای تومانی، «ذخایر الماس» در دنیای الماس');
{
  ...
  ok(/WALLET_LABELS = \[\.\.\.allLabels\(l => l\.buttons\.coinShop\), '💎 کیف الماس', '💎 الماس فروشی'\]/.test(SRC),
  ...
  // ⚠️ v3.81.2: این ادعا هم مثل دوقلویش در check-ux-menu روی جدولِ `packs` نشسته بود که
  // هیچ‌جا رندر نمی‌شد. حالا از `PACK_NAMES` می‌خواند، یعنی همان چیزی که کاربر می‌بیند.
  ok(/PACK_NAMES = \{[^}]*gold: 'بسته ویژه'/s.test(LOC), 'نامِ بسته‌ی وسط در locale «بسته ویژه» است');
...
console.log('\n▶ تعدادِ الماس بدونِ جداکننده‌ی هزارگان (v3.77.0، گزارشِ مالک)');
{
  ...
  /* 🐛 و پیامِ **تأییدِ شارژ** که در همان PR جا افتاده بود (v3.78.0): تنها جایی که خریدِ
   * «بسته جاودان» به کاربر اعلام می‌شود. کاربر روی فاکتور «۱۰۰۰» می‌دید و یک پیام بعد
   * «۱٬۰۰۰» — همان یک عدد، دو نگارش. هر دو عددِ این پیام شمارشِ الماس‌اند، پس هر دو
   * بدونِ جداکننده‌اند؛ دو نگارشِ متفاوت در **یک پیام** از هر دو حالت بدتر است. */
  const approved = L.wallet.coinsApproved(1000, 1030, cur);
  ok(approved.includes('۱۰۰۰') && !approved.includes('۱٬۰۰۰'),
    `پیامِ تأییدِ شارژ: تعدادِ الماسِ اضافه‌شده بدونِ جداکننده (شد: «${approved.split('\n')[0]}»)`);
  ok(approved.includes('۱۰۳۰') && !approved.includes('۱٬۰۳۰'),
    'و موجودیِ جدید هم در همان پیام هم‌شکل است');
}
