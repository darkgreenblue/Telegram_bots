...
console.log('\n▶ بسته‌های خریدِ سکه');
{
  ...
  // ⚠️ از v3.75.0: پنج بسته (سه‌تای قدیم + افسانه‌ای + جاودان). نگرانیِ «کاربر نباید
  // بینِ گزینه‌های زیاد گیر کند» حالا با آزمایشِ pack_reveal_v1 حل می‌شود (سه‌تای اول
  // پیش‌فرض، دو تای گران‌تر پشتِ یک دکمه‌ی کشف)، نه با سقفِ سختِ سه‌تا.
  ok(packs.length === 5, `پنج بسته (${packs.length}) — سه‌تای قدیم + افسانه‌ای + جاودان`);
