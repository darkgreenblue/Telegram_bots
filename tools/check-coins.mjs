...
console.log('\n▶ نامِ «جای موجودی»: کیف‌پول در دنیای تومانی، «کیف الماس» در دنیای الماس');
{
  ...
  // نامِ خودِ جا از «موجودی الماس» به **«کیف الماس»** رفت (تصمیمِ مالک ۱۴۰۵/۰۵/۲۸).
  ok(/const purse = \(cur\) => \(cur\?\.on \? `کیف \$\{cur\.name\}` : 'کیف‌پول'\)/.test(LOC),
  ...
  const ALLOWED_PURSE = [
    "cur?.on ? `کیف ${cur.name}` : 'کیف‌پول'",           // خودِ تعریفِ purse (تک‌منبع)
  ...
  ok(/coinShop: '💎 کیف الماس'/.test(LOC), 'دکمه‌ی کیبوردِ دنیای الماس «کیف الماس» است (نه «الماس فروشی»)');
