...
const PROMPT_VARIANTS = {
  /* ═══ v5 — فرضیه‌ی دورِ ۱۴۰۵/۰۶/۲۵ ═══
   *
   * از خطِ پایه‌ی همین روز (۲۸ نوبت، دو دور). چهار یافته، و هر چهار یک ریشه‌ی مشترک
   * دارند: **خطِ آخر یک درخواستِ اجازه است** («می‌خوای … کنیم؟»). چون برچسبِ دکمه
   * «همان دعوتِ خطِ آخر از زبانِ کاربر» تعریف شده، تنها چیزی که می‌شود ساخت جوابِ
   * کاربر به آن درخواست است: یا تأیید («آره بریم سراغش») یا یک تودوی اول‌شخص
   * («معیارها را مشخص کنم»، «متن را بنویس»). هیچ‌کدام سؤال نیستند، و همین است که
   * مالک گرفت: «انگار جمله‌ای نیست که کاربر خودش می‌نوشت».
   *
   * و یک عارضه‌ی بزرگ‌ترِ همان ریشه در دورِ `--tap`: چون ربات دستورکار را پیشنهاد
   * می‌دهد، ۸ تپِ پیاپی گفتگو را از فال بیرون بُرد (تا نوشتنِ متنِ پیام برای طرفِ
   * مقابل) و قلابِ بی‌لنگر به ۴/۸ رسید. کاربر بابتِ هر نوبت یک الماس داد.
   *
   * چهار تغییر، هر کدام با سنجه‌ی خودش:
   *   ۱) خطِ آخر **خبری** می‌شود، نه درخواستِ اجازه  ⟵ سنجه: `fuStyle` و «بیش از یک ؟»
   *   ۲) `follow_up` صریحاً **سؤالِ خودِ کاربر** تعریف می‌شود  ⟵ سنجه: `fuStyle`
   *   ۳) سه فکتِ **روی صفحه** (☰، دکمه‌ی پایان، دکمه‌ی فالِ تازه) به مدل داده می‌شود
   *      ⟵ سنجه: جوابِ C3 (خط ۱ و ۲ خطِ پایه راه‌حلِ کامپیوتریِ ساختگی دادند)
   *   ۴) سؤالِ «چقدر واقعیه/قطعیه» جوابِ صریح می‌گیرد  ⟵ سنجه: C2 نوبتِ ۱
   * ⚠️ عدد و قیمت و الماس همچنان **نمی‌داند** — آن تصمیمِ ثبت‌شده‌ی v3.89.0 است و
   * دست نمی‌خورد (پاداشِ دعوت در تاریخِ همین ربات سه بار عوض شده). */
  v5: (s) => {
    let o = s;
    o = rep(o, '- یک زاویه‌ی تازه‌ی مشخص را جلویش بگذار و بپرس سراغش برویم یا نه؛ یا چیزی را که خودش نیم‌بند گفت نام ببر و پیشنهاد بده بازش کنیم.',
    ...
    o = rep(o, '- follow_up: برچسبِ یک دکمه، **از زبانِ خودِ او** و انگار خودش می‌پرسد؛ همان دعوتِ خطِ آخر. حداکثر ۶۰ نویسه، یک خط، بدونِ گیومه. فقط وقتی هیچ ادامه‌ای نمانده خالی بگذار.',
      '- follow_up: **سؤالِ بعدیِ خودِ او** دربارهٔ همان زاویه‌ی خطِ آخر، دقیقاً همان‌طور که خودش تایپش می‌کرد: اول‌شخص، و حتماً یک سؤالِ کامل که به «؟» تمام شود. ممنوع: قولِ پرسیدن («سؤالمو می‌پرسم»)، تأییدِ حرفِ تو («آره بریم سراغش»)، تودوی خودش («معیارها رو مشخص کنم») و دستور به تو («… رو بنویس»). حداکثر ۶۰ نویسه، یک خط، بدونِ گیومه. فقط وقتی هیچ ادامه‌ای نمانده خالی بگذار.');
    o = rep(o, '- پیش‌بینیِ مرگ و بیماری و فاجعه، وعده‌ی تضمینی، و ترساندن ممنوع.',
    ...
    o = rep(o, '- فکتِ محصولی را از خودت نساز. قیمتِ فال، تعدادِ الماس، راهِ گرفتنش، پاداشِ دعوت، روشِ شارژ و هر قاعده‌ی دیگرِ این ربات را **نمی‌دانی**. پرسید، هیچ عددی نگو و حدس نزن؛ بگو دقیقش دستِ پشتیبانی است و پرچمِ needs_support را روشن کن.',
    ...
    o = rep(o, '- wants_end: وقتی true که خودش نشان بدهد می‌خواهد تمام کند (خداحافظی، تشکرِ پایانی، «فعلاً کافیه»).',
