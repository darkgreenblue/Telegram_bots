 ...
 * کلمه‌ی بی‌ربط را یکی کند (تطبیقِ کاذب عدد را خوش‌بینانه خراب می‌کند). */
{
  ...
  /* 🐛 گاردِ نویسه‌ی بیگانه‌ی **فارسی**، که تا ۱۴۰۵/۰۶/۲۸ اصلاً وجود نداشت (`alien: []`).
   * باگِ واقعی: یک فالِ فارسی «ولی این خشم事实证明 که این کشش از سرِ عشقه» تولید کرد و
   * «بی‌ایراد» گزارش شد؛ فقط خواندنِ انسانی گرفتش. رشته‌ی زیر **عینِ همان خروجی** است،
   * نه یک نمونه‌ی ساختگی، تا اگر الگو روزی تنگ شود همان باگِ واقعی قرمز بدهد. */
  ok(Array.isArray(fa.alien) && fa.alien.length >= 2,
    `فارسی گاردِ نویسه‌ی بیگانه دارد (${(fa.alien || []).length} الگو)`);
  const alienHit = (t) => (fa.alien || []).some(a => a.re.test(t));
  ok(alienHit('ولی این خشم事实证明 که این کشش از سرِ عشقه'),
    'کنترلِ مثبت: همان نویسه‌ی چینیِ باگِ واقعی گرفته می‌شود');
  ok(alienHit('این کارت می‌گه که вы باید صبر کنی'), 'نویسه‌ی سیریلیک هم گرفته می‌شود');
  /* و دو کنترلِ **معکوس**، چون الگوی پهن‌شده روی متنِ سالم هزینه دارد (درسِ ۲۹ قرمزِ
   * کاذبِ پرتغالی): متنِ فارسیِ سالم و واژه‌ی لاتینِ مشروع نباید قرمز بدهند. */
  ok(!alienHit('کارت اول ده سکه می‌گه ریشه‌ی این دل‌تنگی به خونه و خانواده‌ست'),
    'متنِ فارسیِ سالم قرمز نمی‌دهد');
  ok(!alienHit('این فال درباره‌ی AI و تصمیمِ خودِ توئه'),
    'واژه‌ی لاتینِ مشروع قرمز نمی‌دهد (قاعده‌ی لاتینِ روسی عمداً پورت نشد)');
