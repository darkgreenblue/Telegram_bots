...
console.log('\n▶ نامِ بسته‌ی وسط');
{
  ...
  ok(/packs:\s*\{[^}]*gold: 'بسته ویژه'/.test(LOC), 'بسته‌ی وسط «بسته ویژه» نام دارد (بود «بسته‌ی الماسی»)');
  ...
  ok(/PACK_STYLE = \{ gold: 'success', magic: 'primary', legend: 'danger' \}/.test(SRC),
    'رنگ‌ها به کلیدِ بسته بسته‌اند نه به نام (ویژه سبز، جادویی آبی، افسانه‌ای قرمز)');
  // «جاودان» عمداً رنگِ رسمی ندارد (هر سه رنگِ Bot API قبلاً مصرف شده)؛ فقط پین می‌کنیم
  // که این خالی‌بودن **عمدی** است، نه فراموشی.
  ok(!/PACK_STYLE = \{[^}]*eternal:/.test(SRC), 'و «جاودان» عمداً در PACK_STYLE نیست (رنگِ چهارمی وجود ندارد)');
...
console.log('\n▶ ناوبریِ یک‌قدمی و ادیت-در-جا (UX v2.3)');
{
  ...
  // ⚠️ v3.75.0: پیشوندِ 👑 برای «جاودان» اضافه شد (تنها راهِ تمایزش، چون رنگِ رسمی ندارد).
  ok(/coinPack: \(p, cur\) => `\$\{p\.key === 'eternal' \? '👑 ' : ''\}\$\{p\.emoji\} \$\{packName\(p\)\}: ➕\$\{fmt\(p\.coins\)\}\$\{cur\.emoji\} \| \$\{fmt\(p\.toman\)\} تومان`/.test(LOC),
    'دکمه‌ی بسته: «🥉 بسته‌ی معمولی: ➕۱۰💎 | ۵۰٬۰۰۰ تومان»، «👑 جاودان» برای بستهٔ آخر');
