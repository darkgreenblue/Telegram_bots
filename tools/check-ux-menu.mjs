...
// سه زبانِ دیگر هم import می‌شوند چون قاعده‌ی بند ۲و/۱ می‌گوید هر تغییرِ رو-به-کاربر
// by default روی همه‌ی زبان‌هاست؛ ادعای تک‌زبانه یعنی سه ربات بی‌گارد بمانند.
...
console.log('\n▶ قرارداد منوی فال (پین اول، «همه فال‌ها» آخر)');
{
  ...
  /* 🎯 خواسته‌ی صریحِ مالک (۱۴۰۵/۰۶/۲۷): زیرِ **منوی کوتاه** تنها دکمه‌ی خروج
   * «مشاهده همه فال‌ها» است و هیچ ردیفِ بازگشتی نمی‌آید. کسی که از منوی کوتاه
   * می‌خواهد برگردد، اول لیستِ کامل را باز می‌کند و ردیفِ بازگشت آن‌جاست.
   * پس این ادعا **هر دو نیمه** را می‌سنجد: آخرین ردیفِ منوی کوتاه، و نبودِ ناوبری. */
  ok(/allSpreadsV2, 'catalog_go'\)\],\s*\n\s*\];/.test(fn),
    '«مشاهده همه فال‌ها» آخرین و تنها دکمه‌ی خروجِ منوی کوتاه است');
  ok(!/navBackRow|navMenuRow/.test(fn),
    '⛔ و هیچ ردیفِ بازگشتی زیرِ منوی کوتاه ساخته نمی‌شود (دو راهِ خروجِ رقیب ممنوع)');
  /* 🔁 کنترلِ مثبت: ادعای منفیِ بالا فقط وقتی معنی دارد که ردیفِ بازگشت **جایی دیگر**
   * واقعاً وجود داشته باشد. اگر روزی `navBackRow` از کلِ مسیرِ منو حذف شود، این ادعا
   * می‌گوید راهِ بازگشت گم شده، نه اینکه «تمیز شده». */
  ok(/navBackRow\(uid, navMenuRow\(\)\)/.test(SRC.slice(SRC.indexOf('const allTopicsKb = (uid)'),
    SRC.indexOf('const allTopicsKb = (uid)') + 600)),
    '🔁 کنترلِ مثبت: ردیفِ بازگشت به لیستِ کاملِ فال‌ها منتقل شده (نه حذف‌شده)');
...
console.log('\n▶ یک استانداردِ واحد برای «وسطِ فلوی باز» (v3.17.0)');
{
  ...
  /* v3.114.0: گاردِ افشا متنِ **خودش** را دارد (`deliverGuard`). متنِ مشترکِ قبلی می‌پرسید
     «ادامه بدی یا بی‌خیالش بشی؟» در حالی که این گارد عمداً انصراف ندارد: وعده‌ی دکمه‌ای
     که روی صفحه نبود (تیکتِ #TRT-1902690343، کاربر دنبالش گشت). */
  ok(/L\.reading\.deliverGuard/.test(d) && !/L\.reading\.openReadingGuard\b/.test(d),
    'گاردِ افشا متنِ بی‌انصرافِ خودش را می‌دهد، نه متنِ «ادامه یا بی‌خیال»');
...
console.log('\n▶ باکسِ نقل‌قولِ موجودی و نگارشِ چسبیده‌ی عدد و واحد (UX v2.3)');
{
     ...
     شکلِ خط را می‌دید و مثلاً وارونه‌شدنِ ترتیب یا نشتِ فالِ گران را نمی‌گرفت. */
  {
    ...
    /* 🔢 خطِ هزینه باید **اندازه‌ی فال** را هم بگوید (خواسته‌ی صریحِ مالک، ۱۴۰۵/۰۶/۲۷).
     *
     * دلیلش محصولی است، نه تزئینی: کاربر دقیقاً **به‌خاطرِ انتخابِ یک فالِ چندکارتی**
     * به این صفحه رسیده، پس عددی که جلویش است باید بگوید بابتِ چند کارت است. بدونش
    ...
    for (const [code, LL] of Object.entries(LOCS)) {
      ...
      const with3 = LL.reading.needBalance({ ...base, size: 3 });
      const none = LL.reading.needBalance({ ...base, size: undefined });
      ok(with10 !== with3 && with10 !== none,
    ...
    ok(L.reading.needBalance({ ...base, size: 10 }).includes('۱۰ کارتی'),
      '🔤 فارسی دقیقاً «۱۰ کارتی» می‌نویسد (رقمِ فارسی، نه لاتین)');
    /* 🔑 و سیمِ اتصال: «مقدار وجود دارد» با «مقدار می‌رسد» یکی نیست (بند ۲و/۶ب ریشه).
     * اندازه از `SPREAD_BY_ID` می‌آید نه از خودِ رکورد، چون ردیفِ `pending_payment`
     * ستونِ اندازه ندارد و آن‌جا `undefined` امن‌تر از یک صفرِ ساختگی است. */
    ok(/size: SPREAD_BY_ID\[reading\?\.type\]\?\.size/.test(SRC),
      '🔑 و `needBalanceText` اندازه را واقعاً پاس می‌دهد (وگرنه همه‌ی ادعاهای بالا آینه‌ی خودشان بودند)');
...
console.log('\n▶ منوی خودکارِ بعد از /start (v3.58.0)');
{
  ...
  ok(/if \(row\) return await ctx\.reply\(L\.reading\.deliverGuard/.test(hs),
