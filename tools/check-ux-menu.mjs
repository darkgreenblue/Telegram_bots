...
//   ۱) **ریاضیِ کارت شانس.** امیدِ ریاضیِ جایزه باید دقیقاً ۱ سکه در روز باشد. این عدد
...
console.log('\n▶ 🍀 کارت شانس — ریاضیِ جایزه (امیدِ ریاضی باید دقیقاً ۱ سکه باشد)');
{
  ...
  ok(coins === 8, `پشتِ ${coins} کارت از ${grid} سکه هست`);
  ...
  ok(Math.abs(E - 1) < 1e-9, `امیدِ ریاضی دقیقاً ۱ سکه در روز است (E=${E.toFixed(6)})`);
  ok(Math.abs(E - picks * coins / grid) < 1e-9, 'فرمولِ ساده و توزیعِ کامل یک عدد می‌دهند');
  ok(P.length - 1 === 3, 'حداکثرِ جایزه ۳ سکه است (خواسته‌ی مالک: ۱ تا ۳)');
  ...
  ok(/const LUCKY_COIN_VALUE = 1 \* COIN_VALUE;/.test(SRC),
    'هر کارتِ سکه‌دار دقیقاً یک سکه است و مقدارش از COIN_VALUE می‌آید');
}
...
console.log('\n▶ 🍀 کارت شانس — گاردهای پول و حالت');
{
  ...
  ok(h.indexOf('stmts.credit.run(LUCKY_COIN_VALUE') < h.indexOf('if (!done)'),
    'سکه لحظه‌ی برگشتنِ هر کارت واریز می‌شود، نه آخرِ بازی (ری‌استارت پول را نمی‌خورد)');
