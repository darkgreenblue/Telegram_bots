...
export default {
   ...
   *
   * 🐛 نسخه‌ی اول این‌جا نوشته بود «برخلافِ پرتغالی ابهامی نیست»، و آن **غلط بود**.
   * ابهامِ ضمیرِ انعکاسی واقعاً نیست (شرط «si» است و انعکاسی «se»)، ولی ابهامِ دومی
   * هست که از قلم افتاد: «si» در اسپانیایی **«آیا»** هم معنی می‌دهد. و چون
   * نتیجه‌ی شرط در این زبان تقریباً همیشه «entonces sabrás si …» می‌شود، آخرین
   * «si»ِ متن همان «آیا»ست نه حرفِ شرط. سنجه که آخرین را می‌گیرد، **بندِ اشتباه**
   * را نمره می‌داد: در دورِ ۱۴۰۵/۰۶/۲۸ دو فال از نُه به همین دلیل ❌ شدند در حالی
   * که شرطِ واقعی‌شان نامِ کارتِ همین فال را می‌بُرد (۵/۹ گزارش شد، ۷/۹ بود).
   *
   * پس همان قیدِ **ابتدای بندِ** پرتغالی این‌جا هم لازم است، با دلیلِ متفاوت:
   * «آیا» همیشه بعد از یک فعل می‌آید («sabrás si»)، شرط بعد از نقطه یا ویرگول.
   * ⚠️ پرتغالی اتفاقاً از این باگ مصون بود چون قیدش را از روزِ اول داشت — یعنی
   * ۹/۹ پرتغالی و ۵/۹ اسپانیایی تا حدی اختلافِ **خط‌کش** بود نه اختلافِ زبان. */
  closingCond: { open: /(?:^|[.;:!?\n]\s*|,\s+)si(?=\s)/gi, sep: /,?\s*entonces\s/i },
  fake: {
