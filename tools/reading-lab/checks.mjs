...
export function checkReading({ llm, rendered, spread, cards, ctx, L }) {
  ...
  {
    const leak = full.match(/حسِ?\s*ناگفته|نشونه[‌\s]?ات\s+این|نشانه[‌\s]?ات\s+این/);
  ...
  {
    const F = /می‌دهد|می‌کند|می‌شود|می‌گوید|می‌آید|می‌رود|هستند|نیستند|نمی‌شوی|می‌شوی|نخواهد|نشان می‌دهد|(?<![آ-یٔ])است(?![آ-یٔ])/g;
    const hits = full.match(F) || [];
    if (hits.length >= 3) issues.push(`لحنِ کتابی به‌جای گفتاری (${hits.length} مورد): «${hits.slice(0, 4).join('، ')}»`);
  }
