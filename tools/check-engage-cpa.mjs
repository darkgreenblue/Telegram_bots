...
console.log('\n▶ ۹) «اکشنِ مفید» فیچرِ خاموش و دُمِ فلوی دیگر را نمی‌شمارد');
{
  /* ⚠️ این بخش عمداً **ساختاری** است، نه آینه‌ای. یک ادعای آینه‌ای («فلان اسم در
     ...
     ⚠️ محدودیتِ صادقانه — این گارد **یک‌طرفه** است. زیرِ جابِ `dashboard` می‌دود
     (`ci.yml`)، و `readersOf` در `ci-changed-bots.mjs` فقط برای `tools/`،
     `.github/workflows/` و `video/` دامنه به ارث می‌دهد، نه برای `bots/`. یعنی:
       ✅ افزودنِ یک رویدادِ دست‌نیافتنی به USEFUL_EVENTS (تغییرِ engage.js) → گرفته
          می‌شود، چون جابِ dashboard اجرا می‌شود. این جهتِ خطرناک است.
       ❌ خاموش‌کردنِ پرچمِ یک رویدادِ **از قبل در لیست** (تغییرِ فقط bots/tarot/) →
          گرفته نمی‌شود، چون جابِ dashboard اصلاً اجرا نمی‌شود.
     عمداً این‌طور ماند: بستنِ جهتِ دوم یعنی جابِ dashboard روی **هر** تغییرِ tarot
     اجرا شود، که دقیقاً همان انفجارِ تعدادِ جاب است که بند ۳ج ریشه با ماتریسِ
     مسیرمحور ۴۵٪ کمش کرد. و ریسکِ باقی‌مانده کوچک است: سه رویدادِ فعلیِ لیست
     (کارت روز، کارت شانس، فالِ کامل) حلقه‌ی اصلیِ محصول‌اند و رفتنشان پشتِ یک
     پرچمِ خاموش تصمیمی نیست که بی‌خبر رخ بدهد. */
