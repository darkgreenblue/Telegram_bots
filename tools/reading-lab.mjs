...
const PROMPT_VARIANTS = {
  ...
  /* 🕯 بازوی «جمع‌بندی باید نامِ کارت را ببرد» — از ۱۴۰۵/۰۶/۲۸ **فقط فارسی**.
   *
   ...
   * دارند: «تو این چند هفته» و «اگر صبر کنی» را می‌شود عیناً زیرِ هر فالِ دیگری
   * گذاشت. اسپکِ فارسی می‌گوید شرط باید «به کارت‌های همین فال بخورد» ولی این یک
   * **توصیف** است، نه چیزی که خودِ مدل بتواند بسنجدش.
   *
   * ⚠️ چهار ردیفِ دیگر (en/ru/pt/es) در همین تاریخ **حذف شدند، نه خاموش** (بند ۹/۰):
   * متنِ برنده به خودِ پرامپتِ آن چهار زبان رفت، پس ردیفشان این‌جا فقط کدِ مرده بود.
   * فارسی تنها زبانی است که قاعده را **ندارد** (استثنای ثبت‌شده، تصمیمِ صریحِ مالک:
   * پرامپتِ رباتِ زنده دست نمی‌خورد)، و همان دورِ جفت‌شده‌ی دومِ فارسی که سندِ ربات
   * شرطِ برگشتنِ قاعده می‌داندش، دقیقاً همین بازو را لازم دارد.
   *
   * متنِ تازه **آینه‌ی دقیقِ** جمله‌ی انگلیسی است (نام را بلند ببر + جمله‌ی تعریفیِ
   * «شرطِ بی‌نامِ کارت یعنی توصیه‌ی عمومی») تا فرضیه همان فرضیه بماند. */
  closinganchor: (sys) => {
    const OLD = 'شرط باید به کارت‌های همین فال و سؤالِ خودش بخورد، نه یک توصیه‌ی عمومی.';
    const NEW = 'شرط باید نامِ یکی از کارت‌های همین فال را صریح ببرد و به سؤالِ خودش چفت شود، نه یک '
      + 'توصیه‌ی عمومی. شرطی که نامِ کارت در آن نباشد، توصیه‌ی عمومی است.';
    /* زبانی که این جمله را ندارد (چهار زبانِ دیگر، که از قبل قاعده را دارند) دست‌نخورده
     * برمی‌گردد و گاردِ «واریانت هیچ تغییری نداد» **بلند** شکست می‌خورد، نه بی‌صدا. */
    return sys.includes(OLD) ? sys.replace(OLD, NEW) : sys;
  },
