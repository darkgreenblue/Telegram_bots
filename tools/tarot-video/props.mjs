...
/**
 * متنِ خوانشِ یک کارت، با همان زنجیره‌ی رباتِ زنده: نرمال‌سازیِ شکلِ خروجی، برداشتنِ
 * برچسبِ ترتیبی که مدل خودش جلوی جمله گذاشته (شماره‌گذاری کارِ کد است)، و پاک‌سازیِ
 * خط تیره.
 *
 * یک تفاوتِ عمدی با ربات: نشانه‌ی جداکننده‌ی سرِ جمله برداشته می‌شود. وقتی مدل نوشته
 * «و کارتِ آخرت — یعنی صبر»، برچسب که می‌رود و خط تیره که ویرگول می‌شود، جمله با
 * «، » شروع می‌شود. در ربات آن ویرگول بینِ برچسب و متن می‌نشیند و دیده نمی‌شود، ولی
 * در ویدیو متن زیرِ برچسبِ خودش تنها می‌ایستد و همان ویرگول شبیهِ خرابی به نظر می‌رسد.
 */
export function cardRead(x) {
  return clean(stripCardLabel(readText(x))).replace(/^[،؛:.\s]+/, '');
}
