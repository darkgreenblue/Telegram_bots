...
/* ── خواننده‌ی مستقلِ مبلغِ حروفی (نوشته‌شده از صفر، بدونِ import از locale) ──────
 * «شصت و سه هزار و پانصد» ⟵ ۶۳۵۰۰.
 *
 * ⚠️ الگوریتمش عمداً با تولیدکننده **فرق** دارد: آن یکی عدد را به گروه‌های مقیاس
 * می‌شکند و هر گروه را به حروف می‌نویسد؛ این یکی واژه‌به‌واژه جلو می‌رود و روی هر
 * واژه‌ی مقیاس، انباشته را ضرب و تخلیه می‌کند. دو مسیرِ متفاوت به یک عدد می‌رسند،
 * پس یک غلطِ مشترک نمی‌تواند در هر دو طرف یکسان تکرار شود و سبز رد شود
 * (بند ۶ب ریشه: گاردِ آینه‌ای فقط آینه‌ی خودش را می‌سنجد).
 *
 * `null` یعنی واژه‌ای دیدم که نمی‌شناسم — عمداً، تا خروجیِ خراب با یک عددِ ساختگی
 * مقایسه نشود. */
const W_SCALE = { هزار: 1e3, میلیون: 1e6, میلیارد: 1e9 };
const W_NUM = {
...
function wordsToNum(text) {
  const toks = String(text).trim().split(/\s+/).filter((t) => t && t !== 'و');
  if (!toks.length) return null;
  let total = 0, cur = 0;
  for (const t of toks) {
    if (W_SCALE[t] !== undefined) { total += (cur || 1) * W_SCALE[t]; cur = 0; continue; }
    if (W_NUM[t] === undefined) return null;
    cur += W_NUM[t];
  }
  return total + cur;
}
...
{
  // کنترلِ مثبت: بدونِ این، یک `wordsToNum` که همیشه null بدهد همه‌ی ادعاهای پایین را
  // بی‌صدا خنثی می‌کرد (بند ۶ب-۲: سبزِ حاصل از نبودِ قرمز هیچ چیز ثابت نمی‌کند).
  ok(wordsToNum('شصت هزار') === 60_000, 'مبلغِ دست‌نویسِ «شصت هزار» خوانده می‌شود');
  ok(wordsToNum('شصت و سه هزار و پانصد') === 63_500, 'و «شصت و سه هزار و پانصد» هم');
  ok(wordsToNum('یک میلیون و چهارصد و نود هزار') === 1_490_000, 'و مقیاسِ میلیون هم');
  ok(wordsToNum('نهصد و نود و نه هزار و نهصد و نود و نه') === 999_999, 'و بدترین حالتِ سه‌رقمی‌های پشتِ‌هم');
  ok(wordsToNum('چیزِ ناشناس') === null, 'و ورودیِ ناشناس null می‌دهد، نه یک عددِ ساختگی');
  // ⚠️ کنترلِ منفیِ خودِ خواننده: اگر ارقام را هم قبول می‌کرد، ادعای «هیچ رقمی در
  // پرانتز نیست» می‌توانست با یک تولیدکننده‌ی رقمی سبز رد شود.
  ok(wordsToNum('۶۰ هزار') === null, 'و شکلِ رقمیِ نسل قبل را دیگر **نمی‌شناسد** (پرانتز باید حروفی باشد)');
}
...
console.log('\n💰 خطِ مبلغ: عددِ کامل اول، شکلِ کوتاه در پرانتز، هر دو از یک مبلغ');
{
  ...
  for (const n of NUMS) {
    ...
    const got = wordsToNum(inParens.replace('تومان', ''));
    if (got !== n) { bad = `${n} ⟵ «${inParens}» ⟵ ${got}`; break; }
  }
  ok(!bad, `هر ${NUMS.length} مبلغ، پرانتزش همان عدد را می‌گوید${bad ? ` (شکست: ${bad})` : ''}`);

  /* 🔤 خواسته‌ی صریحِ مالک (۱۴۰۵/۰۶/۲۶): داخلِ پرانتز **هیچ رقمی** نباشد، کلِ مبلغ به
   * حروف. این ادعای مستقلی از تطابقِ عددیِ بالاست: یک تولیدکننده‌ی ترکیبی («۵۰ هزار»)
   * می‌توانست از آن یکی سبز رد شود، چون عددش درست بود — فقط شکلش خواسته نشده بود. */
  let withDigit = null;
  for (const n of NUMS) {
    const inParens = (lineOf(render(n, purchase), 'مبلغ:').match(/\(([^)]+)\)/) || [])[1] || '';
    if (/[0-9۰-۹]/.test(inParens)) { withDigit = `${n} ⟵ «${inParens}»`; break; }
  }
  ok(!withDigit, `و در هیچ‌کدام حتی یک رقم داخلِ پرانتز نیست${withDigit ? ` (شکست: ${withDigit})` : ''}`);
  ...
  ok(line.includes('(شصت هزار تومان)'), 'مبلغِ حروفی داخلِ پرانتز است، نه جای اصلی');
  ok(lineOf(render(63_500, purchase), 'مبلغ:').includes('*۶۳٬۵۰۰ تومان* (شصت و سه هزار و پانصد تومان)'),
  ...
  /* اعدادِ واقعیِ کاتالوگِ امروز — چون همین‌ها را کاربرِ واقعی می‌بیند، نه مرزهای نظری. */
  for (const [n, want] of [[15_000, 'پانزده هزار'], [60_000, 'شصت هزار'],
    [150_000, 'صد و پنجاه هزار'], [1_500_000, 'یک میلیون و پانصد هزار']]) {
    ok(lineOf(render(n, purchase), 'مبلغ:').includes(`(${want} تومان)`),
      `مبلغِ کاتالوگیِ ${n.toLocaleString('en-US')} ⟵ «${want} تومان»`);
  }
