 ...
 *   ۲) خطِ مبلغ: بولتِ خنثی + عددِ **کامل** + **همان عدد** به شکلِ کوتاه، داخلِ پرانتز.
...
console.log('\n🔹 خطِ مبلغ: عددِ کامل اول، شکلِ کوتاه در پرانتز، هر دو از یک مبلغ');
{
  ...
  /* ایموجیِ خنثی: خواسته‌ی صریحِ مالک «یه ایموجیِ خنثی مثل بولت رنگی، نه ایموجیِ پول».
   * ادعا **هر دو جهت** را می‌گیرد: بولت باشد، و هیچ ایموجیِ پولی برنگردد. */
  ok(line.trim().startsWith('🔹'), 'خطِ مبلغ با بولتِ رنگی شروع می‌شود');
  for (const money of ['💵', '💰', '💸', '🤑', '💲', '💴', '💶', '💷', '🪙']) {
    ok(!line.includes(money), `و هیچ ایموجیِ پولی (${money}) در آن نیست`);
  }
}
