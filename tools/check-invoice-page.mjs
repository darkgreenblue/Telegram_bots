...
/* ══ 💳 شماره کارت: دش فقط نمایشی است ═══════════════════════════════════════
 * خواسته‌ی مالک: «۴ رقم ۴ رقم با دش، ولی دش توی کپی‌هاش نیاد».
 *
 * ⚠️ این همان خانواده‌ی باگی است که ریپو بارها خورده (ریال/تومانِ رسید، «۶۰٬۰۰۰
 * ستاره»): چیزی که **نمایش** داده می‌شود و چیزی که **واقعاً استفاده** می‌شود از دو
 * جا می‌آیند و یکی‌شان دیر یا زود عوض می‌شود. این‌جا خطرش عملی است: کاربر عددِ
 * دش‌دار را در اپِ بانکی بزند و کارت پیدا نشود.
 *
 * پس ادعای مرکزی یک تساویِ سه‌طرفه است: نمایشِ بدونِ دش == `CARD_NUMBER` == چیزی که
 * دکمه‌ی کپی حمل می‌کند. */
console.log('\n💳 شماره کارت: دش نمایشی، کپی تمیز');
{
  ...
  /* ⚠️ بک‌تیک ممنوع است و دلیلش رفتاری است نه سلیقه‌ای: متنِ داخلِ بک‌تیک با تپ
   * **عیناً همان چیزی را کپی می‌کند که نشان می‌دهد**، پس دش هم کپی می‌شد. */
  ok(!/`\d{4}-/.test(body) && !body.includes(`\`${CARD}\``),
    'شماره کارت داخلِ بک‌تیک نیست (وگرنه تپ، دش را هم کپی می‌کرد)');

  // و تنها مسیرِ کپی، دکمه است — که مستقلِ از متنِ پیام، ارقامِ خام را حمل می‌کند.
  const copyRow = SRC.match(/const cardCopyRow = \(\) => (.+);/)?.[1] || '';
  ok(/copy_text: \{ text: CARD_NUMBER \}/.test(copyRow),
    'دکمه‌ی کپی مستقیماً CARD_NUMBER را کپی می‌کند، نه نسخه‌ی نمایشی');
  ok(!/cardFmt|-/.test(copyRow.replace('=>', '')), 'و هیچ دشی واردِ فیلدِ کپی نمی‌شود');
