...
for (const file of files) {
   ...
   *
   * 🐛 و یک ادعای غلط که همین‌جا نوشته بود و ۱۴۰۵/۰۶/۲۷ باطل شد: «روی مقدارهای
   * بارگذاری‌شده کار می‌کند، پس کامنت‌های کد طبیعتاً بیرون می‌مانند». برای **رشته**
   * درست است، برای **تابع** نه: `Function.prototype.toString` سورسِ کامل را با
   * کامنت‌هایش برمی‌گرداند. پس یک کامنتِ توضیحیِ حاویِ «—» بالای بدنه‌ی یک تابعِ
   * locale چک را قرمز می‌کرد، در حالی که هیچ کاربری آن نویسه را نمی‌دید — قرمزِ کاذب،
   * و از آن بدتر: سندنویسیِ داخلِ کد به یک باگ تبدیل می‌شد. این **ششمین** بارِ ثبت‌شده‌ی
   * همان تله در این ریپو است (v3.56.0، v3.64.0، check-price-ladder، check-llm-usage،
   * v3.81.0)، پس قاعده‌اش دیگر بحث ندارد: هر ادعای متنی کامنت‌ها را **قبل** از سنجش
   * حذف می‌کند. */
  const stripComments = (s) => s
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    // فقط کامنتِ **سرِ خط**: `//`ِ وسطِ خط می‌تواند داخلِ یک رشته باشد (مثلاً `https://`)
    // و بریدنش یعنی متنِ واقعیِ کاربر از دامنه‌ی سنجش بیرون بیفتد.
    .replace(/^[ \t]*\/\/.*$/gm, ' ');
  const userFacing = Object.entries(mod).filter(([k]) => k !== 'prompts');
  const texts = [];
  const collect = (v) => {
    if (typeof v === 'string') texts.push(v);
    else if (typeof v === 'function') { try { texts.push(stripComments(v.toString())); } catch {} }
  ...
  /* 🔁 و چون این گارد حالا چیزی را **کنار می‌گذارد**، باید ثابت شود کورش نکرده‌ایم
   * (بند ۶ب-۲ ریشه: سبزیِ حاصل از نبودِ قرمز هیچ چیز ثابت نمی‌کند). دو کنترل، در دو
   * جهتِ مخالف: متنی که کاربر واقعاً می‌بیند باید هنوز گرفته شود، و کامنت نباید. */
  if (code === 'fa') {
    const inText = () => `یک ${'—'} در متن`;
    const inComment = () => {
      // این کامنت عمداً یک — دارد
      return 'متنِ سالم';
    };
    const seen = (fn) => stripComments(fn.toString()).includes('—');
    if (!seen(inText)) fail('🔁 کنترلِ مثبت شکست: «—» داخلِ متنِ واقعیِ تابع باید دیده شود');
    if (seen(inComment)) fail('🔁 کنترلِ منفی شکست: «—» داخلِ کامنت نباید شمرده شود');
  }
