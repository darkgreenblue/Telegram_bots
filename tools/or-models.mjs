...
//
// ✅ تصحیحِ ۱۴۰۵/۰۶/۲۸: این‌جا قبلاً نوشته بود «باید روی رانرِ گیت‌هاب اجرا شود چون
// خروجیِ شبکه‌ی محیطِ توسعه بسته است». آن جمله **غلط** است و سنجیده شد: همین endpoint
// از محیطِ توسعه با HTTP 200 جواب می‌دهد. نگه‌داشتنِ یک جمله‌ی باطل در کامنت بی‌ضرر
// نیست؛ همان الگویی است که یک بار باگِ کیبورد را پوشاند (سندِ داخلِ کد به‌جای هشدار،
// پوششِ باگ شد). چیزی که واقعاً runner-only است **کلید** است، نه فهرستِ مدل‌ها.
//
...
/* ═══ 🔑 سقفِ مدل‌های رایگان، **فقط وقتی کلید هست** ═══
 *
 * چرا لازم شد (۱۴۰۵/۰۶/۲۸، آزمایشِ DeepSeekِ رایگان): واریانتِ `:free` سقفِ
 * «درخواست در دقیقه» و «درخواست در روز» دارد. آن سقف، مستقل از کیفیت، **حقِ وتو**
 * روی انتشار دارد: مدلی که کیفیتش عالی باشد ولی سقفش کمتر از ترافیکِ واقعیِ ربات،
 * روی پروداکشن نمی‌رود.
 *
 * ⚠️ و عددش را **نمی‌شود از مستندات خواند**: جدولِ سقف‌ها در صفحه‌ی Limits سلولِ
 * عددیِ سمتِ سرور ندارد و اعداد سمتِ کلاینت تزریق می‌شوند. تنها عددِ معتبر برای حسابِ
 * خودمان `free_model_daily_requests` در پاسخِ `GET /api/v1/key` است. طبقِ بند ۹/۰الف
 * ریشه، «خوانده‌نشده» یعنی نقل نمی‌شود؛ پس این‌جا خوانده می‌شود.
 *
 * هیچ‌چیزِ سری چاپ نمی‌شود: نه کلید، نه بخشی از کلید. فقط سقف و شمارنده و برچسب. */
const KEY = (process.env.OPENROUTER_API_KEY || '').trim();
if (KEY) {
  try {
    const r = await fetch('https://openrouter.ai/api/v1/key', { headers: { Authorization: `Bearer ${KEY}` } });
    if (!r.ok) {
      console.log(`\n🔑 سقفِ کلید خوانده نشد: HTTP ${r.status}`);
    } else {
      const d = (await r.json()).data || {};
      const f = d.free_model_daily_requests || {};
      console.log('\n🔑 وضعیتِ کلید (بدونِ هیچ مقدارِ سری):');
      console.log(`   برچسب: ${d.label ?? '—'} | سقفِ اعتبارِ کلید: ${d.limit ?? 'بی‌سقف'} | باقی‌مانده: ${d.limit_remaining ?? 'بی‌سقف'}`);
      console.log(`   مصرف: کل ${d.usage ?? '—'} | امروز ${d.usage_daily ?? '—'} | این ماه ${d.usage_monthly ?? '—'}`);
      console.log(`   🆓 درخواستِ مدلِ رایگان در این روزِ UTC: ${f.used ?? '—'} از ${f.limit ?? '—'} (باقی: ${f.remaining ?? '—'})`);
      console.log('   ⚠️ سقفِ «در دقیقه» در این پاسخ نیست و فقط با خوردن به 429 دیده می‌شود.');
    }
  } catch (e) {
    console.log(`\n🔑 سقفِ کلید خوانده نشد: ${e.message}`);
  }
} else {
  console.log('\n🔑 سقفِ مدلِ رایگان چاپ نشد: کلیدی در محیط نبود (این قدم اختیاری است).');
}
