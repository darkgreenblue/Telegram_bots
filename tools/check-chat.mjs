...
console.log('\n▶ ۱۳) پرامپتِ گفتگو');
{
  ...
  /* ⚠️ از v3.116.0 سقف روی **پرامپت + یادآوریِ قالب** است، چون هر دو در پیشوندِ ثابتِ
...
console.log('\n▶ ۲۰) سؤالِ پیشنهادی، پایانِ مکالمه و دکمه‌ی منو');
{
  ...
  /* 🧭 `/menu` از ۱۴۰۵/۰۶/۲۷ نیت **می‌گیرد**، و این وارونه‌شدنِ یک تصمیمِ ثبت‌شده است
   * نه سهو. استدلالِ قبلی («جوابش همان پیامِ پیش‌فرضِ `replyCanceled` است، پس ثبتِ نیت
   * فقط یک مسیرِ موازیِ اضافه می‌ساخت») دقیقاً تا وقتی درست بود که `/menu` همان پیام را
   * می‌داد. حالا به `navToMenu` وصل است و جوابش «برگشتیم به منوی اصلی» + کیبوردِ ماندگار
   * است؛ بدونِ نیت، کاربری که وسطِ گفتگو `/menu` می‌زند و بعد گفتگو را می‌بندد پیامِ
   * ادامه می‌گیرد نه منو — یعنی همان باگی که این تغییر برای بستنش بود (بند ۹ب/۶ ریشه). */
  ok(intentOf('/menu', null) === IN.MENU, '🧭 و `/menu` نیتِ منوی اصلی می‌گیرد (جوابش دیگر پیامِ پیش‌فرض نیست)');
  ok(/\[INTENT\.MENU\]:\s*\(ctx\) => navToMenu\(ctx\)/.test(CODE),
    '🔑 و بازپخشِ همان نیت دقیقاً همان کاری را می‌کند که خودِ دستور می‌کرد');
  ...
  /* 🧭 `/menu` از ۱۴۰۵/۰۶/۲۷ دقیقاً همان «◀️ بازگشت به منوی اصلی» است (خواسته‌ی صریحِ
   * مالک). قبلاً `deliverKeyboard` + `sendContinuePrompt` می‌زد، یعنی کیبورد را بی‌صدا
   * می‌داد و بعد پیامِ **ادامه** را می‌فرستاد، نه پیامِ منو.
   *
   * ادعا عمداً **دو تکه** است، چون یکی‌شان به‌تنهایی پوچ است: صدا زدنِ `navToMenu` ثابت
   * نمی‌کند کیبورد می‌رود، و بودنِ `mainKeyboard` در `navToMenu` ثابت نمی‌کند `/menu` به
   * آن‌جا می‌رسد. با هم یعنی «کیبوردِ ماندگار واقعاً تحویل می‌شود» (بند ۹ب-۳ ریشه). */
  const menuCmd = bodyOf(CODE, "bot.command('menu'") || '';
  ok(/return navToMenu\(ctx\);/.test(menuCmd),
    '🧭 `/menu` همان `navToMenu` است (کپیِ دستیِ گاردها ساخته نشد)');
  ok(!/deliverKeyboard|sendContinuePrompt/.test(menuCmd),
    '⛔ و دیگر پیامِ «ادامه» نمی‌فرستد (آن پیامِ منو نبود)');
  ok(/return ctx\.reply\(L\.reading\.backToMenu, mainKeyboard\(uid\)\);/.test(bodyOf(CODE, 'async function navToMenu(') || ''),
    '🔑 و `navToMenu` کیبوردِ ماندگار را **واقعاً** تحویل می‌دهد، با همان متنِ دکمه‌ی بازگشت');
...
console.log('\n▶ ۲۱) فقط آخرین پیام دکمه دارد + کفِ محتوا');
{
  ...
  /* از v3.116.0 همان یک تلاش کمبودِ پیشنهاد و حرفِ خطر را هم می‌گیرد؛ برچسبِ هزینه
...
/* ═══ ۲۲) پرامپتِ v6: خطِ آخر «پیشنهاد» است + سقفِ برچسبِ دکمه (v3.104.0) ════════
 *
 * چهار موردِ بازخوردِ مالک روی ترنسکریپتِ تستِ دستیِ خودش، و همه از یک ریشه: v5 خطِ
 * آخر را **خبری** کرده بود («خبری بنویس، نه درخواستِ اجازه») تا برچسبِ دکمه سؤال شود،
 * و همان قاعده «می‌رسیم به…» و «آماده می‌کنم» را تولید کرد. تصمیمِ صریحِ او: خطِ آخر
 * همیشه **پیشنهاد**، دکمه همیشه از پرسپکتیوِ **کاربر**.
 *
 * ⚠️ ادعای مرکزیِ این بخش **معکوس** است: جمله‌ی v5 باید واقعاً **نباشد**. بدونِ آن،
 * برگشتنِ سهویِ همان یک جمله بی‌صدا از CI رد می‌شود و کلِ v6 را باطل می‌کند
 * (بند ۶ب-۲ ریشه: سبزیِ حاصل از نبودِ قرمز هیچ چیز ثابت نمی‌کند). */
console.log('\n▶ ۲۲) پرامپتِ v6: پیشنهادِ خطِ آخر و سقفِ برچسب');
{
  const p = (FA.match(/chatSystem: `([\s\S]*?)`,\n/) || [])[1] || '';
  ok(!!p, 'پرامپتِ گفتگو از سورس برداشته شد');

  // ۱) شکلِ خواسته‌شده در پرامپت اعلام شده.
  ok(/═══ خطِ آخر: پیشنهاد ═══/.test(p), '🎁 بلوکِ خطِ آخر «پیشنهاد» نام دارد، نه «دعوتِ ادامه»');
  ok(/با یک \*\*پیشنهاد\*\* تمام می‌شود/.test(p) && /بدونِ استثنا/.test(p),
    '🎁 و «هر جواب، بدونِ استثنا» را صریح می‌گوید (خواسته‌ی مالک: تو همه پیام‌ها)');
  ok(/می‌خوای …؟/.test(p) && /اگه بخوای می‌تونم/.test(p),
    '🎁 هر دو شکلِ تأییدشده‌ی مالک در پرامپت‌اند');
  ok(/همیشه از زبانِ \*\*تو\*\*/.test(p),
    '🎁 و پرسپکتیو صریح است (خطِ آخر از زبانِ تاروت‌خوان، نه کاربر)');

  /* ۲) ادعاهای **معکوس** — هر سه شکلی که مالک به‌نام رد کرد باید در پرامپت ممنوع باشند،
   * و جمله‌ی v5 باید رفته باشد. */
  ok(!/خبری بنویس، نه درخواستِ اجازه/.test(p),
    '🔁 جمله‌ی v5 («خبری بنویس، نه درخواستِ اجازه») حذف شده — همان چیزی که «می‌رسیم به…» را می‌ساخت');
  for (const [lbl, re] of [['خبرِ کارِ خودت', /می‌نویسم/], ['«می‌رسیم به»', /می‌رسیم به/],
                           ['«می‌تونیم … کنیم»', /می‌تونیم … کنیم/], ['زبانِ کاربر', /می‌خوام بدونم/],
                           ['سؤالِ خالی', /سؤالِ خالیِ بی‌پیشنهاد/]]) {
    ok(re.test(p), `⛔ شکلِ ردشده‌ی «${lbl}» در پرامپت ممنوع شده`);
  }

  /* ۳) سقفِ برچسب: عددِ پرامپت باید **همان ثابتِ کد** باشد، وگرنه پرامپت و تورِ ایمنی
   * بی‌صدا واگرا می‌شوند. پرامپت رقمِ فارسی می‌نویسد، پس مقایسه روی همان خط انجام
   * می‌شود نه روی رشته‌ی خام. */
  const faDigits = (n) => String(n).replace(/\d/g, (d) => '۰۱۲۳۴۵۶۷۸۹'[Number(d)]);
  ok(new RegExp(`حداکثر ${faDigits(chat.CHAT_FU_PROMPT_MAX)} نویسه`).test(p),
    `✂️ سقفِ برچسب در پرامپت همان CHAT_FU_PROMPT_MAX است (${chat.CHAT_FU_PROMPT_MAX})`);
  ok(chat.CHAT_FU_PROMPT_MAX < chat.CHAT_FU_BTN_MAX,
    '✂️ و سقفِ پرامپت سخت‌گیرتر از سقفِ نمایشیِ کد است (کارِ اصلی را پرامپت می‌کند)');
  ok(/جوابِ پیشنهادِ تو نیست، سؤالِ خودش است/.test(p),
    '🏷 و رابطه‌ی دکمه با خطِ آخر صریح است (برچسب سؤالِ کاربر است، نه تأییدِ پیشنهاد)');

  /* ۴) `chatBtnLabel` — **رفتاری**، با تابعِ واقعیِ محصول. */
  const at = 'ا'.repeat(chat.CHAT_FU_BTN_MAX);
  ok(chat.chatBtnLabel(at) === at, `✂️ برچسبِ دقیقاً ${chat.CHAT_FU_BTN_MAX}نویسه‌ای دست نمی‌خورد (کنترلِ مثبت)`);
  const long = 'قبل از قبول فرصت دوباره چه چیزایی رو چک کنم و چطور بسنجم؟';
  const cut = chat.chatBtnLabel(long);
  ok(cut.length <= chat.CHAT_FU_BTN_MAX, `✂️ برچسبِ بلند به ≤${chat.CHAT_FU_BTN_MAX} می‌رسد (${cut.length})`);
  ok(cut.endsWith('…'), '✂️ و با «…» تمام می‌شود، پس بریدگی به کاربر اعلام می‌شود');
  ok(!/\s…$/.test(cut) && long.startsWith(cut.slice(0, -1)),
    '✂️ و بریدن روی مرزِ کلمه است و پیشوندِ همان برچسب می‌ماند');
  const oneWord = 'ا'.repeat(chat.CHAT_FU_BTN_MAX * 2);
  ok(chat.chatBtnLabel(oneWord).length <= chat.CHAT_FU_BTN_MAX,
    '✂️ تک‌کلمه‌ی بلندتر از سقف هم بریده می‌شود (نه اینکه از گارد رد شود)');

  /* ۵) 🔑 مهم‌ترین ادعای این بخش: بریدن **فقط نمایشی** است.
   * دکمه برچسبِ بریده می‌گیرد ولی ردیفِ دیتابیس سؤالِ **کامل** را، وگرنه تپِ دکمه
   * سؤالِ نصفه می‌پرسد و بازگوییِ `blockquote`ِ v3.96.0 هم نصفه می‌شود. */
  ok(/Markup\.button\.callback\(chatBtnLabel\(followUp\), `chat_ask:\$\{aId\}`\)/.test(SRC),
    '✂️ برچسبِ دکمه از chatBtnLabel رد می‌شود');
  const ins = (SRC.match(/stmts\.insertChatMsg\.run\([^)]*\)/) || [''])[0];
  ok(/,\s*followUp,/.test(ins) && !/chatBtnLabel/.test(ins),
    '🔑 ولی ردیفِ دیتابیس `followUp`ِ **کامل** را می‌گیرد، نه برچسبِ بریده');
  ok(/const q = row\.follow_up;/.test(SRC),
    '🔑 و تپِ دکمه همان سؤالِ کاملِ ردیف را می‌پرسد (نه متنِ روی دکمه)');

  /* ۶) 📊 دورِ اولِ اندازه‌گیریِ v6 نشان داد ۶ نقض از ۱۹ باقی مانده و **۵تایشان یک
   * کلاس بودند**: جوابِ فکتِ محصولی / ارجاع به پشتیبانی / اشاره به یک دکمه. آن جواب‌ها
   * کوتاه و درست‌اند و مدل آن‌جا را استثنا می‌فهمید، چون بلوک‌های «مرزها» و «فالِ تازه»
   * می‌گویند «همین را بگو». حالا پرامپت صریح می‌گوید استثنا نیست.
   *
   * ⚠️ و تنها استثنای واقعی هم صریح نوشته شده: `wants_end`. این معافیت **لازم** است،
   * چون «پیشنهادِ تازه» به کسی که خداحافظی کرده دقیقاً همان «گفتگو را جمع نکن» را از
   * جهتِ مخالف نقض می‌کند. */
  ok(/فکتِ محصولی، ارجاع به پشتیبانی و اشاره به یک دکمه هم استثنا نیست/.test(p),
    '🎁 پرامپت صریح می‌گوید جوابِ محصولی/پشتیبانی/دکمه هم از پیشنهاد معاف نیست');
  ok(/تنها استثنا: خودش دارد تمام می‌کند \(wants_end\)/.test(p),
    '🎁 و تنها استثنایش `wants_end` است، با همین نام');

  /* ۷) 🔁 و همان قاعده باید در **سنجه** هم برقرار باشد، وگرنه گزارش و پرامپت دو چیزِ
   * متضاد می‌خواهند. این‌جا فقط **ساختاری** سنجیده می‌شود (که معافیت روی `flags.end`
   * بنشیند و نه روی پرچم‌های دیگر)؛ اجرای رفتاریِ هر چهار ترکیبِ پرچم کنترلِ ششمِ
   * `chat-lab --fake` است، چون آن ماژول به locale بارشده وابسته است. */
  const CHK = strip(fs.readFileSync(new URL('./reading-lab/chat-checks.mjs', import.meta.url), 'utf8'));
  ok(/const endTurn = !!\(flags && flags\.end\);/.test(CHK),
    '🎁 معافیتِ سنجه روی `flags.end` می‌نشیند');
  ok(/noOffer = !isOffer && !endTurn;/.test(CHK),
    '🎁 و تنها همان یک شرط معافش می‌کند');
  ok(!/noOffer[\s\S]{0,120}flags\.(support|newReading)/.test(CHK),
...
/* ═══ ۲۳) v3.116.0: پیشنهاد در همه‌ی جواب‌ها + حرفِ خطر فقط در خطرِ واقعی ═══════
...
console.log('\n▶ ۲۳) v3.116.0: پیشنهادِ اجباری و گاردِ بحران');
{
