...
/* ══ 🗄 انتخابِ دیتابیس: یک منبع، نه دو کپی ══════════════════════════════════
 * 🐛 باگِ واقعیِ ۲۰ شهریور ۱۴۰۵: انتخابِ «بزرگ‌ترین دیتابیس» در ۱۲ شهریور برای
 * `db-query` اصلاح شد ولی دوقلویش در `exp` روی `ls data/*.db | head -1` (اولین به
 * ترتیبِ الفبا) ماند. چون تاروت چهار دیتابیس دارد، آزمایشِ `price_ladder_p1` در
 * `bot-es.db` ساخته شد — رباتی که اصلاً ریلِ قیمتِ تومانی ندارد. صفر خطا، صفر اثر.
 * پس ادعا روی **تک‌منبع بودن** می‌نشیند، نه روی درستیِ هر کپی: تا وقتی دو نقطه‌ی
 * انتخاب وجود داشته باشد، این باگ دوباره ممکن است. */
const dbQueryCase = (YML.match(/\n\s+db-query\)([\s\S]*?)\n\s+;;/) || ['', ''])[1];
ok(/pick_db\(\) \{/.test(YML), 'یک helperِ pick_db برای انتخابِ فایلِ .db تعریف شده');
ok(/ls -S data\/\*\.db/.test(YML), 'و بزرگ‌ترین فایل را برمی‌دارد (نه اولین به ترتیبِ الفبا)');
/* ⚠️ «انتخاب» با «چاپِ نامزدها» یکی نیست: هر دو اکشن عمداً `ls -Sla data/*.db` را برای
 * **نمایش** هم دارند تا انتخابِ اشتباه دیده شود. نسخه‌ی اولِ همین ادعا هر دو را یکی
 * شمرد و سه قرمزِ کاذب داد. پس الگو روی چیزی می‌نشیند که واقعاً انتخاب می‌کند:
 * یک `ls` روی `data/*.db` که به `head -1` می‌رسد. */
/* ⚠️ و کامنت‌ها اول حذف می‌شوند. کامنتِ توضیحیِ خودِ `db-query` عبارتِ باگِ قدیمی را
 * **نقل** می‌کند («این‌جا `ls data/*.db | head -1` بود»)، و شمردنش به‌عنوان کد یک
 * قرمزِ کاذبِ دیگر داد. همان تله‌ی ثبت‌شده‌ی v3.56.0/v3.64.0، این‌بار در YAML. */
const noHash = (t) => (t || '').split('\n').filter(l => !/^\s*#/.test(l)).join('\n');
const SELECT_RE = /data\/\*\.db[^\n]*\|\s*head -1/g;
const picks = (noHash(YML).match(SELECT_RE) || []).length;
ok(picks === 1, `⭐ فقط **یک** نقطه‌ی انتخابِ دیتابیس در کلِ فایل هست (دیده شد: ${picks})`);
for (const [name, body] of [['exp', expCase], ['db-query', dbQueryCase]]) {
  ok(/DBFILE=\$\(pick_db\)/.test(body), `اکشنِ ${name} از همان helper می‌خواند`);
  ok(!new RegExp(SELECT_RE.source).test(noHash(body)), `و ${name} کپیِ محلیِ انتخاب را ندارد`);
}
// و ادعای معکوس: اگر کسی helper را به «اولین به ترتیبِ الفبا» برگرداند، قرمز شود.
ok(!/ls data\/\*\.db 2>\/dev\/null \| head -1/.test(YML),
  'شکلِ باگ‌دارِ قدیمی (ls data/*.db | head -1) هیچ‌جا برنگشته');
