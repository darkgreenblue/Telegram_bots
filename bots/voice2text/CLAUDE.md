...
- **گاردِ اصلی سمتِ داشبورد است** (`lib/bots.js`): ردیفِ رجیستریِ این ربات
  `adminActions: ['approve', 'reject']` دارد — یعنی **فهرستِ کاملِ** آن‌چه sweepش اجرا
  می‌کند. داشبورد هیچ اکشنی بیرون از این فهرست برایش صف نمی‌کند و دکمه‌اش را هم رندر
  نمی‌کند.
- ⚠️ **چرا فهرست، نه بولین (۱۴۰۵/۰۶/۱۵):** نسخه‌ی اولِ این گارد یک بولینِ
  `creditQueue` بود که فقط `credit`/`credit_paid` را می‌پوشاند، پس صفحه‌ی **پشتیبانی**
  که شش اکشن صف می‌کند (`force_approve`, `approve_accounting`, `debit`,
  `unlock_reading`, …) کاملاً بی‌گارد مانده بود. تنها چیزی که این ربات را نجات می‌داد
  این بود که جدولِ `admin_actions`ش ستون‌های `user_id/amount/ref_id/note` را ندارد و
  `assertColumns` خطا می‌داد — یک گاردِ **اتفاقی** که اولین مهاجرتِ افزایشیِ بی‌ضررِ
  بعدی خاموشش می‌کرد. حالا گارد روی خودِ قرارداد نشسته، نه روی شکلِ جدول.
- ℹ️ عارضه‌ی همان شکلِ جدول: دکمه‌ی «❌ رد» در صفحه‌ی **پشتیبانی** برای این ربات با
  خطای `schema-guard` می‌خورد (آن صفحه هر شش ستون را می‌خواهد). عمداً درست نشده چون
  **بی‌صدا نیست** و راهِ کارآمدش هست: تأیید/ردِ رسید از صفحه‌ی `/finance` که INSERT
  باریک دارد و مثل همیشه کار می‌کند.
...
- اگر روزی خواستی این ربات هم شارژِ دستیِ داشبورد را بگیرد: یک شاخه‌ی `credit`/
  `credit_paid` به sweep اضافه کن، ستون‌های `user_id/amount/ref_id/note` را به
  `admin_actions` اضافه کن، و نامشان را به `adminActions` در رجیستری ببر. چکِ
  `tools/check-credit-queue.mjs` **هر دو جهت** را می‌سنجد: نامِ بی‌شاخه و شاخه‌ی
  بی‌نام هر دو قرمزند (پرچمِ بی‌دلیل خاموش هم باگ است، فقط جهتش برعکس).
