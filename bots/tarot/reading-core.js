...
/* ═══ کارتِ سنگینی که **نیامده** ═══ */
// چرا از کد می‌آید نه از مدل: نسخه‌ی قبلی مدل را آزاد گذاشته بود خودش بگوید «چه کارتِ
// بدی نیامده»، و نتیجه در ۷ فال از ۷ یک قالبِ واحد شد — «خوشبختانه کارت‌های سنگین
// نیومدن» با اسم‌های مبهم و غیرقابلِ‌ابطال. در خوانشِ واقعیِ انسانی این جمله **مشخص**
// بود («کارت فروپاشی نیفتاد کلا»): یک کارتِ نام‌برده. تفاوتِ «معنادار» و «Barnum»
// دقیقاً همین است.
//
// ما دقیقاً می‌دانیم کدام ۷۸ کارت هست و کدام‌ها کشیده شده‌اند، پس انتخاب کارِ کد است
// (قاعده: هرچه کد می‌تواند تضمین کند نباید به مدل سپرده شود). مدل فقط **معنیِ نیامدنش
// برای سؤالِ او** را می‌نویسد.
//
// انتخاب قطعی است (هشِ همان کارت‌های کشیده‌شده) تا بعد از ری‌استارت هم عوض نشود.
const HEAVY = ['m16', 'm13', 'm15', 's10', 's03', 's09', 'p05', 'c05', 'm18', 'w10'];

export function absentHeavy(cards) {
  const drawn = new Set(cards.map(c => c.key));
  const pool = HEAVY.filter(k => !drawn.has(k));
  if (!pool.length) return null;                       // بعید (۱۰ کارتِ سنگین، ۱۰ کشیده) ولی ممکن
  const pick = pool[seedToInt(cards.map(c => c.key).join(',')) % pool.length];
  return { fa: CARD_BY_KEY[pick].fa, means: CARD_BY_KEY[pick].up.slice(0, 2).join('، ') };
}
...
export function buildReadingCtx({ user, spread, question, cards, focusKey, L, prev = [], kbOn = false, name = '' }) {
  return {
    ...
    // کارتِ سنگینی که نیامده — انتخابش با کد است، نه مدل
    'کارتِ سنگینی که نیامده': absentHeavy(cards),
    previous: prev.map(r => ({
      'نوع فال': r.type,
      'خلاصه': r.summary,
