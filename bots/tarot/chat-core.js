 ...
 *
 * ⚠️ `sys` در ۱۴۰۵/۰۶/۲۲ از ۳۵۰۰ به ۳۹۰۰ رفت و **همان ۴۰۰ کاراکتر از `hist` کم شد**،
...
export const CHAT_BUDGET = {
  sys: 3900,      // پرامپتِ سیستم (سقفِ سورس هم همین است؛ چکِ CI قفلش کرده)
  ...
  hist: 5000,     // تاریخچه‌ی گفتگو (فشرده + نوبت‌های خام)
  ask: 700,       // سؤالِ فعلی
...
/** شرطِ پذیرشِ خروجیِ مدل در نسلِ **متنِ خام**. عمداً کمینه: کیفیتِ محتوا کارِ
 *  آزمایشگاه است نه retryِ کور.
 *  ⚠️ تا وقتی پرامپت JSON نمی‌دهد این هنوز validateِ زنده است؛ همان کامیتی که اسکیما
 *  را به پرامپت اضافه می‌کند جایش را به `chatOutOk` می‌دهد. جدا کردنشان عمدی است:
 *  اگر این یکی همین حالا به پاکت گره می‌خورد، مدل که هنوز متنِ خام می‌دهد از هر
 *  چهار پله‌ی `CHAT_PLAN` رد می‌شد و **هر نوبتِ گفتگو** به ریفاند می‌افتاد. */
...
/* مسیرِ ۲ — **گاردِ فکتِ پولی**، و دامنه‌اش دقیقاً برعکسِ چیزی است که اول ساختم.
 *
 * گارد فقط وقتی اعمال می‌شود که مدل جواب را **عادی** فرض کرده باشد: نه پشتیبانی،
 * نه فالِ تازه. استدلالِ مالک: اگر خودش پرچمِ پشتیبانی را زده، یعنی تشخیص داده سؤال
 * محصولی است و لابد درست هم رفتار کرده؛ گارد آن‌جا بی‌مورد است. حالتِ خطرناک دقیقاً
 * جایی است که مدل **متوجه نشده** و با اطمینان قیمت می‌گوید.
 *
 * و دیتای دورِ ۱۴۰۵/۰۶/۲۲ همین را تأیید می‌کند: آن‌جا هیچ پرچمی در کار نبود و مدل با
 * اطمینانِ کامل گفت «فال گرفتن از من رایگانه» — یعنی دقیقاً همین شاخه‌ی «عادی».
 *
 * درمانش **retry** است، نه متنِ آماده (تصحیحِ صریحِ مالک). */
export const chatFactOk = (p) => !p || p.support || p.newReading || !moneyClaimIn(p.text);

/** شرطِ پذیرشِ نهاییِ نسلِ پاکت = هر دو مسیر. جدا نگه داشته شده‌اند تا هر کدام ادعای
 *  CI خودش را داشته باشد و دامنه‌ی یکی بی‌صدا روی دیگری نلغزد.
 *  ⚠️ هنوز هیچ مصرف‌کننده‌ی زنده‌ای ندارد — با همان کامیتی وصل می‌شود که اسکیمای JSON
 *  را به پرامپت اضافه می‌کند. تا آن لحظه این فایل فقط توابعِ خالصِ تست‌شده را دارد و
 *  رفتارِ ربات بیت‌به‌بیت دست‌نخورده است. */
export const chatOutOk = (out) => {
  const p = parseChatOut(out);
  return !!p && chatFactOk(p);
};
