...
- **منو:** دکمه‌ی کیبوردِ «🎁 سرگرمی‌های رایگان» (ردیفِ دوم؛ فقط وقتی فلگ روشن) → منوی اینلاین: کارت روز (`daily_go`) + فال حافظ (`hafez_go`) + استخاره (`estekhare_go`) + کوییز (`quiz_go`). نقطه‌ی ورودِ دوم: ردیفِ `freemenu` در پی‌والِ ناکافی‌بودنِ موجودی. فازهای بعدی (فال قهوه/کتابخانه کارت) به همین منو اضافه می‌شوند.
- **آنالیتیکس (اختصاصی، افزایشی):** `free_menu_opened`، `hafez_taken` (n/via)، `estekhare_taken` (outcome/via)، `quiz_started`، `quiz_done` (props: card)، و `first_value` (once، `via` = hafez/estekhare/quiz). قیف: free_menu_opened → hafez_taken/estekhare_taken/quiz_done → (CTA) opentopic/spread → paywall_shown.
...
`users` (balance، state، focus_area، session_json، memory_json، milestone/push، `last_daily_date`/`daily_streak`، `last_hafez_date` (فال حافظ روزانه)، `estekhare_date`/`estekhare_count` (سقفِ ۳/روز استخاره)، `last_quiz_date` (کوییزِ ماهی‌یک‌بار)، **first_source/first_payload** اتریبیوشن write-once)، `readings` (منبع حقیقت فال: seed، cards_json، llm_json، status: pending_payment/started/delivered/canceled/refunded)، `payments` (+ `reminded_at`)، `admin_actions` (صف تأیید/رد رسید از داشبورد)، `discount_codes`، `discount_uses`، `referrals`، `card_files` (کش file_id تصاویر)، `daily_texts`، **`events`** (آنالیتیکس مشترک — پایین).
