...
- **منو:** دکمه‌ی کیبوردِ «🎁 سرگرمی‌های رایگان» (ردیفِ دوم؛ فقط وقتی فلگ روشن و دیتا موجود) → منوی اینلاین: کارت روز (`daily_go` موجود) + فال حافظ (`hafez_go`). نقطه‌ی ورودِ دوم: ردیفِ `freemenu` در پی‌والِ ناکافی‌بودنِ موجودی (`freeMenuRow()`). فازهای بعدی (استخاره/کوییز/فال قهوه/کتابخانه کارت) به همین منو اضافه می‌شوند.
- **آنالیتیکس (اختصاصی، افزایشی):** `free_menu_opened`، `hafez_taken` (props: n=شماره‌ی غزل، via)، و `first_value` (once، `via:'hafez'`). قیف: free_menu_opened → hafez_taken → (CTA) opentopic → paywall_shown.
- متن‌ها همه در `locales/fa.js` (بلوک‌های `freeMenu` و `hafez` + دکمه‌ها)؛ زبان بدون `hafez.js` = فیچر خودکار غیرفعال.

## دیتابیس (`data/bot-<LOCALE>.db`)
`users` (balance، state، focus_area، session_json، memory_json، milestone/push، `last_daily_date`/`daily_streak`، `last_hafez_date` (فال حافظ روزانه)، **first_source/first_payload** اتریبیوشن write-once)، `readings` (منبع حقیقت فال: seed، cards_json، llm_json، status: pending_payment/started/delivered/canceled/refunded)، `payments` (+ `reminded_at`)، `admin_actions` (صف تأیید/رد رسید از داشبورد)، `discount_codes`، `discount_uses`، `referrals`، `card_files` (کش file_id تصاویر)، `daily_texts`، **`events`** (آنالیتیکس مشترک — پایین).
