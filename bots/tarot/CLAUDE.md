...
- **هدیه‌ی خوش‌آمد ندارد** (حذف شد؛ مسیر رایگان فقط کارت روز است) و **حداقل مبلغ شارژ هم ندارد** (هر عدد مثبتی پذیرفته می‌شود). `REFERRAL_BONUS=10٬000` دوطرفه، `DAILY_CODE=20٪ تا سقف 100٬000` (کد شخصی تک‌مصرف بدون انقضا؛ بعد از اولین کارت روز صادر و بعد از هر کارت روز تا مصرف یادآوری می‌شود؛ با ریست تست پاک و دوباره صادر می‌شود)، `MILESTONE_DAYS=14`، پوش حداکثر هفته‌ای یک‌بار.
...
- CTA دوگانه در استارت: کارت روز رایگان (قلاب اصلی ورود) + فال گذشته/حال/آینده (محبوب‌ترین).
