...
- **فال قهوه‌ی سؤال‌محور (فاز ۵):** آیتمِ پنجمِ منو (`coffee_go`). مدلِ فال‌الدین: ۳ سؤالِ قاب‌بندی (دغدغه‌ی امروز / خودانگاره / حسِ غالب) × ۴ گزینه = ۶۴ ترکیب؛ هر پاسخ یک «نقشِ فنجان» (`options[i].s`)؛ خوانش = چیدنِ سه نقش کنارِ هم + یک بختِ پایانیِ **hash-selected** از `closings` (تنوع بدونِ انفجارِ محتوا). **حالتِ بدونِ state** (`coffee:<answers>` در callback_data). روزی یک‌بار (ستونِ افزایشیِ `last_coffee_date`؛ گارد هم در start و هم روی نتیجه). فضاسازی: «فنجونت رو برمی‌گردونم رو نعلبکی...» + مکث. CTA به `opentopic`. رویداد `coffee_taken` (props: combo). همه در locale (بلوکِ `coffee`).
- **منو:** دکمه‌ی کیبوردِ «🎁 سرگرمی‌های رایگان» (ردیفِ دوم؛ فقط وقتی فلگ روشن) → منوی اینلاین: کارت روز (`daily_go`) + فال حافظ (`hafez_go`) + استخاره (`estekhare_go`) + کوییز (`quiz_go`) + فال قهوه (`coffee_go`). نقطه‌ی ورودِ دوم: ردیفِ `freemenu` در پی‌والِ ناکافی‌بودنِ موجودی. فازِ بعدی (کتابخانه‌ی ۷۸ کارت) به همین منو اضافه می‌شود.
- **آنالیتیکس (اختصاصی، افزایشی):** `free_menu_opened`، `hafez_taken` (n/via)، `estekhare_taken` (outcome/via)، `quiz_started`، `quiz_done` (card)، `coffee_taken` (combo)، و `first_value` (once، `via` = hafez/estekhare/quiz/coffee). قیف: free_menu_opened → hafez/estekhare/quiz/coffee → (CTA) opentopic/spread → paywall_shown.
- متن‌ها همه در `locales/fa.js` (بلوک‌های `freeMenu`، `hafez`، `estekhare`، `quiz` + دکمه‌ها) و دیتای per-کارت در `quiz.js`/غزل‌ها در `hafez.js`؛ استخاره دیتای درون‌locale دارد.

## دیتابیس (`data/bot-<LOCALE>.db`)
`users` (balance، state، focus_area، session_json، memory_json، milestone/push، `last_daily_date`/`daily_streak`، `last_hafez_date` (فال حافظ روزانه)، `estekhare_date`/`estekhare_count` (سقفِ ۳/روز استخاره)، `last_quiz_date` (کوییزِ ماهی‌یک‌بار)، `last_coffee_date` (فال قهوه‌ی روزانه)، **first_source/first_payload** اتریبیوشن write-once)، `readings` (منبع حقیقت فال: seed، cards_json، llm_json، status: pending_payment/started/delivered/canceled/refunded)، `payments` (+ `reminded_at`)، `admin_actions` (صف تأیید/رد رسید از داشبورد)، `discount_codes`، `discount_uses`، `referrals`، `card_files` (کش file_id تصاویر)، `daily_texts`، **`events`** (آنالیتیکس مشترک — پایین).
