...
- **منو:** دکمه‌ی کیبوردِ «🎁 سرگرمی‌های رایگان» (ردیفِ دوم؛ فقط وقتی فلگ روشن) → منوی اینلاین: کارت روز (`daily_go`) + فال حافظ (`hafez_go`) + استخاره (`estekhare_go`). نقطه‌ی ورودِ دوم: ردیفِ `freemenu` در پی‌والِ ناکافی‌بودنِ موجودی (`freeMenuRow()`). فازهای بعدی (کوییز/فال قهوه/کتابخانه کارت) به همین منو اضافه می‌شوند.
- **آنالیتیکس (اختصاصی، افزایشی):** `free_menu_opened`، `hafez_taken` (props: n، via)، `estekhare_taken` (props: outcome، via)، و `first_value` (once، `via:'hafez'`/`'estekhare'`). قیف: free_menu_opened → hafez_taken/estekhare_taken → (CTA) opentopic/spread → paywall_shown.
- متن‌ها همه در `locales/fa.js` (بلوک‌های `freeMenu`، `hafez`، `estekhare` + دکمه‌ها)؛ زبان بدون `hafez.js` = فال حافظ خودکار غیرفعال (استخاره دیتای درون‌locale دارد پس مستقل است).

## دیتابیس (`data/bot-<LOCALE>.db`)
`users` (balance، state، focus_area، session_json، memory_json، milestone/push، `last_daily_date`/`daily_streak`، `last_hafez_date` (فال حافظ روزانه)، `estekhare_date`/`estekhare_count` (سقفِ ۳/روز استخاره)، **first_source/first_payload** اتریبیوشن write-once)، `readings` (منبع حقیقت فال: seed، cards_json، llm_json، status: pending_payment/started/delivered/canceled/refunded)، `payments` (+ `reminded_at`)، `admin_actions` (صف تأیید/رد رسید از داشبورد)، `discount_codes`، `discount_uses`، `referrals`، `card_files` (کش file_id تصاویر)، `daily_texts`، **`events`** (آنالیتیکس مشترک — پایین).
