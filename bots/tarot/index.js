 ...
 * ⛔ **فاز ۲ (بازوی `cheap`) عمداً شروع نشد — خواسته‌ی صریحِ مالک: «فعلاً تستِ جدیدی
 * روی قیمت نمی‌خواهیم».** `cheap` و کلِ این زیرساخت (`PRICE_LADDERS`, `priceArm`,
 * `exposePrice`) عمداً پاک نشدند: آزمایشِ بعدی (اگر/وقتی مالک خواست) دقیقاً همین
 * `price_ladder_p2` است، `control`(=floor امروز) در برابرِ `cheap`، بدونِ نیازِ به
 * هیچ تغییرِ کد — فقط ساختن و start از داشبورد. */
