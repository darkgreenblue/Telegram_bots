...
/* 💎 شمارنده‌ی زنده‌ی الماس، خطِ آخرِ هر جوابِ گفتگو (خواسته‌ی صریحِ مالک ۱۴۰۵/۰۶/۱۹):
 * یک باکسِ نقل‌قول که **فقط** عدد و ایموجی دارد، بدونِ هیچ کلمه‌ای. چون هر سؤال یک
 * الماس می‌دهد، عدد هر نوبت یکی کم می‌شود و کاربر هزینه‌اش را همان‌جا می‌بیند.
 *
 * ⚠️ عدد **بعد از** همه‌ی جابه‌جایی‌های پولِ همین نوبت خوانده می‌شود، نه اولش. کسر قبل
 * از فراخوانیِ مدل انجام شده (بند ۹ ریشه) و ریفاندِ کفِ محتوا هم **قبل از** ارسال
 * می‌نشیند، پس خواندنِ موجودی درست سرِ ارسال تنها جایی است که عدد راست می‌گوید: جوابِ
 * نازکی که الماسش برگشته باید عددِ برگشته را نشان بدهد، نه عددِ کسرشده.
 *
 * ⚠️ و این پیام از این به بعد **HTML** می‌رود، چون `blockquote` تنها راهِ باکس در Bot
 * API است. یعنی متنِ مدل باید `esc` شود؛ یک `<` فرار کرده کافی است که تلگرام **کلِ
 * پیام را رد کند** و جوابی که پولش داده شده گم شود. برای همین ارسال دو پله دارد
 * (پایین) و پله‌ی دوم عیناً همان ارسالِ خامِ دیروز است.
 *
 * رول‌بکِ یک‌خطی: `false` ⟵ نه باکسی ساخته می‌شود و نه HTML ای، بیت‌به‌بیت مثل قبل. */
const CHAT_BALANCE_BOX = true;
...
const needBalanceText = (uid, reading) =>
  L.reading.needBalance({
    ...
    price: reading.price,
    cur: curOf(uid),
    // تعدادِ کارتِ همین چیدمان، برای خطِ «هزینه‌ی فال N کارتی». از `SPREAD_BY_ID`
    // می‌آید نه از خودِ رکورد، چون رکوردِ `pending_payment` ستونِ اندازه ندارد.
    // چیدمانِ ناشناخته ⟵ `undefined` ⟵ locale عدد را اصلاً چاپ نمی‌کند.
    size: SPREAD_BY_ID[reading?.type]?.size,
  });
...
const INTENT = {
  ...
  SETTINGS: 'settings',
  CHAT:    'chat',
  // 🧭 «برگشت به منوی اصلی» — نیتِ `/menu` وقتی گاردی جلویش را می‌گیرد (v3.99.0).
  MENU:    'menu',
};
...
const INTENT_REPLAY = {
  ...
  // ⚠️ همان تابعِ دکمه‌ی بازگشت، نه یک کپی: بازپخشِ نیت هرگز نباید با تپِ واقعی واگرا شود.
  [INTENT.MENU]:    (ctx) => navToMenu(ctx),
};
...
async function blockDuringOpenPay(ctx, intent, intentArg = 0) {
  ...
  // ✅ «تکمیل پرداخت» بالای «انصراف» (خواسته‌ی صریحِ مالک، ۱۴۰۵/۰۶/۲۷).
  //
  // تا امروز تنها دکمه‌ی زیرِ این پیام **انصراف** بود، یعنی تنها اقدامِ ممکنِ کاربر
  // ترکِ خرید — همان ایرادی که v3.95.0 روی یادآوریِ فاکتور گرفت و این‌جا جا مانده
  // بود. متنِ پیام از کاربر می‌خواهد «تکمیلش کن»، ولی هیچ دکمه‌ای برای تکمیل نداشت.
  //
  // 🔑 عمداً همان `pay_resume:` است، نه یک مسیرِ تازه: آن هندلر از قبل دقیقاً همین
  // کار را می‌کند (پیامِ فعلی را پاک می‌کند، فاکتورِ **همان** ردیف را دوباره می‌فرستد،
  // `session.paymentId` و استیتِ `pay_receipt` را برمی‌گرداند) و هیچ فاکتورِ تازه‌ای
  // نمی‌سازد، پس شماره‌ی پرداخت همان می‌ماند و رسیدِ واریزشده روی همان ردیف می‌نشیند.
  // ساختنِ مسیرِ دومِ موازی روی ریلِ پول دقیقاً همان چیزی است که بند ۹ب/۶ منع می‌کند.
  //
  // ⚠️ رسید در **هر دو** مسیر کار می‌کند و این نیازی به کدِ تازه ندارد: گارد فقط وقتی
  // فعال می‌شود که `issuedInvoiceOf` یک فاکتورِ زنده پیدا کند، و `processReceipt` هم
  // از همان مسیر (سشن یا `pendingReceiptPayment`) همان ردیف را پیدا می‌کند. پس چه
  // کاربر زیرِ همین پیام رسید بفرستد و چه بعد از تپِ «تکمیل پرداخت» زیرِ فاکتور.
  await ctx.reply(L.errors.openInvoice, Markup.inlineKeyboard([
    [Markup.button.callback(L.buttons.completePayment, `pay_resume:${pid}`)],
...
function paymentFlowAllowsCallback(state, data) {
  if (!data) return false;
  // 🚪 دو راهِ خروجِ پیامِ گاردِ «فاکتور باز داری»، هر دو **بی‌قیدِ استیت**:
  // `pay_exit` انصرافِ صریح، و `pay_resume` بازگرداندنِ خودِ فاکتور.
  //
  // ⚠️ `pay_resume` قبلاً فقط در `pay_receipt` مجاز بود و تا وقتی تنها صداکننده‌اش
  // یادآوریِ شبانه بود مسئله‌ای نداشت. از ۱۴۰۵/۰۶/۲۷ دکمه‌اش زیرِ خودِ گارد هم هست،
  // و گارد در `pay_discount` هم شلیک می‌شود (کاربرِ وسطِ واردکردنِ کدِ تخفیف). آن‌جا
  // میدل‌ور دکمه را می‌بلعید و **همان پیام دوباره** می‌آمد: حلقه‌ی بی‌پایانی از همان
  // خانواده‌ی تیکتِ #TRT-8976388520 (بند ۹ب/۶ ریشه). خودِ هندلر مالکیت و وضعیت را
  // دوباره می‌سنجد و استیت را درست می‌کند، پس بی‌قید بودنش این‌جا امن است.
  if (/^(pay_exit|pay_resume):\d+$/.test(data)) return true;
  if (state === 'pay_amount') {
...
function chatExitIntent(txt, cb) {
  ...
  // 🧭 `/menu` از ۱۴۰۵/۰۶/۲۷ نیت می‌گیرد. تا قبل از آن عمداً نمی‌گرفت و استدلالش هم
  // درست بود: جوابِ خودش **همان** پیامِ پیش‌فرضِ `replyCanceled` بود، پس ثبتِ نیت فقط
  // یک مسیرِ موازیِ اضافه می‌ساخت. ولی همان روز `/menu` به `navToMenu` وصل شد و جوابش
  // دیگر آن پیام نیست؛ بدونِ این ردیف، کاربری که وسطِ گفتگو `/menu` می‌زد و بعد گفتگو
  // را می‌بست، به‌جای منوی اصلی همان پیامِ ادامه را می‌گرفت — یعنی دقیقاً باگی که این
  // تغییر برای بستنش بود، از درِ گفتگو برمی‌گشت (هم‌خانواده‌ی بند ۹ب/۶ ریشه).
  if (txt === '/menu') return INTENT.MENU;
...
function falMenuKb(uid) {
  ...
  return [
    ...
    // 🎯 آخرین (و تنها) دکمه‌ی خروجیِ منوی کوتاه (خواسته‌ی صریحِ مالک، ۱۴۰۵/۰۶/۲۷).
    //
    // ⛔ ردیفِ بازگشت عمداً این‌جا **نیست**. منوی کوتاه سه موضوع بیشتر ندارد و پایینش
    // دو دکمه‌ی هم‌شکلِ پشتِ‌سرهم («مشاهده همه فال‌ها» و «بازگشت…») دو راهِ خروجِ رقیب
    // می‌ساخت، درست جایی که کاربر باید **یک** انتخاب ببیند: یا یکی از سه فال، یا دیدنِ
    // بقیه. راهِ بازگشت از بین نمی‌رود؛ یک قدم عقب‌تر می‌رود (منوی بلند، پایین) و
    // کیبوردِ ماندگار هم مثل همیشه سرِ جایش است، پس این بن‌بست نیست (بند ۹ب/۱ ریشه).
...
/* 🧭 `/menu` = دقیقاً همان «◀️ بازگشت به منوی اصلی» (خواسته‌ی صریحِ مالک، ۱۴۰۵/۰۶/۲۷).
 *
 * تا امروز این دستور `deliverKeyboard` + `sendContinuePrompt` می‌زد، یعنی کیبورد را
 * بی‌صدا تحویل می‌داد و بعد پیامِ «برای جواب دادن به سؤالاتی که جوابش رو نمی‌دونی من
 * همیشه اینجام!» را می‌فرستاد. آن پیامِ **ادامه** است نه پیامِ منو، و کاربری که `/menu`
 * می‌زند انتظارِ همان چیزی را دارد که دکمه‌ی بازگشت می‌دهد: منوی اصلیِ باز، و جمله‌ی
 * «برگشتیم به منوی اصلی 🌳».
 *
 * 🔑 عمداً همان `navToMenu` است، نه یک کپیِ دیگر: آن تابع از قبل هر سه گاردِ فلوی باز
 * را دارد (فاکتور، افشا، کارتِ شانس)، فالِ پرداخت‌نشده را لغو می‌کند، و **نقطه‌ی مجازِ
 * صدورِ کیبورد** است. کپیِ دستی‌اش دقیقاً همان چیزی است که یک بار در خودِ `navToMenu`
 * باگِ حلقه‌ی #TRT-8976388520 را ساخت (کامنتِ بالای همان تابع). */
...
async function runChatTurn({ uid, r, text, msgId, price, send, step, typingCtx = null }) {
  ...
  try {
    ...
    step?.('chat_reply');
    /* 💎 خطِ آخر: شمارنده‌ی الماس در باکسِ نقل‌قول (ثابتِ `CHAT_BALANCE_BOX` بالا).
     *
     * ⚠️ موجودی **این‌جا** خوانده می‌شود، بعد از بلوکِ کفِ محتوا. کسر قبل از فراخوانیِ
     * مدل انجام شده و ریفاندِ جوابِ نازک هم چند خط بالاتر، پس این تنها نقطه‌ای است که
     * عدد با دفتر می‌خواند. خواندنش اولِ تابع یعنی جوابِ ریفاندشده عددِ کسرشده را نشان
     * بدهد — دقیقاً همان کلاسِ «عدد درست، معنیِ دروغ» که این ریپو بارها هزینه‌اش را داده.
     *
     * ⚠️ و ارسال **دو پله** است، نه یکی: پله‌ی اول HTML (تنها راهِ باکس) با متنِ
     * `esc`شده، و اگر تلگرام با **۴۰۰** ردش کرد پله‌ی دوم همان ارسالِ خامِ بی‌باکسِ
     * دیروز است. بدترین حالتِ این طراحی «باکس نیامد» است، نه «جوابِ پول‌داده گم شد».
     * فقط ۴۰۰ فالبک می‌گیرد و بقیه‌ی خطاها پرتاب می‌شوند: خطای شبکه ممکن است پیام را
     * رسانده و جوابش را گم کرده باشد، و تلاشِ دوم آن‌جا پیامِ **تکراری** می‌سازد. */
    const box = CHAT_BALANCE_BOX ? L.chat.balanceBox(getBalance(uid), curOf(uid)) : '';
    let sent = null;
    if (box) {
      try {
        sent = await send(`${esc(reply)}\n\n${box}`, kb, { html: true, plain: reply });
      } catch (e) {
        if (Number(e?.response?.error_code) !== 400) throw e;
        logErr('chat balance box rejected:', e.message); // ⟵ پله‌ی دوم پایین
      }
    }
    if (!sent) sent = await send(reply, kb);
...
async function resumePendingChat(uid, via = 'purchase') {
  ...
  return runChatTurn({
    ...
    /* ⚠️ `opts.html` هم مثل `kb` باید پاس داده شود، وگرنه باکسِ موجودی در **این** مسیر
     * تگِ خام چاپ می‌کند — و این مسیر همان جوابِ بعد از شارژ است، یعنی دقیقاً جایی که
     * کاربر تازه پول داده و عددِ تازه‌اش را می‌خواهد ببیند.
     * ⚠️ و `opts.plain` برای تایم‌لاین است نه برای تلگرام: `logPush` باید **متنِ خوانا**
     * را ثبت کند، وگرنه بازپخشِ مسیر پر از `<blockquote>` و `&amp;` می‌شود و پیامی که
     * بند ۲الف ریشه واجب کرده در تایم‌لاین باشد، عملاً ناخوانا ثبت می‌شود. */
    send: async (t, kb, opts = {}) => {
      const sent = await bot.telegram.sendMessage(uid, t, {
        ...extra,
        ...(opts.html ? { parse_mode: 'HTML' } : {}),
        ...(kb ? { reply_markup: kb.reply_markup } : {}),
      });
      ...
      logPush(db, uid, opts.plain || t, { isAdmin: isAdmin(uid), label: 'chat_resume' });
      return sent;
    },
  });
}
