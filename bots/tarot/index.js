...
/* ⚠️ `lremind:` هم این‌جاست و دلیلش مشخص است: بعد از کارتِ شانس، جوابِ سؤالِ معلق
 * می‌رسد و استیت `chatting` می‌شود، در حالی که دکمه‌ی «🔔 فردا یادآوری کن» همان بالا
 * روی صفحه است. آن دکمه هیچ‌جا نمی‌برد (یک تاگل است)، پس گارد گرفتنش فقط گیج‌کننده
 * است. قاعده: اکشنی که کاربر را از گفتگو **بیرون نمی‌برد** گارد نمی‌خورد. */
...
bot.action(/^lremind:([01])$/, async (ctx) => {
  ...
  /* 🌙 ⚠️ این شرط عمداً **نامتقارن** است، و تقارنش یک باگِ زنده بود (v3.85.0).
   *
   * تا v3.28.0 یادآوریِ شبانه opt-**out** بود و `daily_reminder_off` تنها ستونی بود که
   * جارو می‌خواند، پس «کاربر یادآوری می‌خواهد» درست یعنی همان ستون را صفر کن. ولی
   * v3.82.0 **معنیِ این جفت را عوض کرد**: حالا صفر بودنِ آن ستون یعنی «کارتِ روز را هم
   * بفرست». یعنی هر کسی که این دکمه را می‌زد بی‌صدا هر دو یادآوری را می‌گرفت و شبش
   * می‌شد کارتِ شانس + زنجیره‌ی کارتِ روز — همان دو پیامِ شبانه‌ای که v3.9.0 دلیلِ
   * بلاک‌شدن ثبتش کرده. روی دیتای زنده ۱۸۲ کاربر از همین راه هر دو کلیدشان روشن شده بود.
   *
   * حالا:
   *  - `on`  = «یادآوریِ **کارتِ شانس** را می‌خواهم» ⟵ فقط `lucky_reminder_on`. کارتِ
   *    روز دست‌نخورده می‌ماند؛ اگر کسی واقعاً هر دو را بخواهد، صفحه‌ی تنظیمات جایش است
   *    (خواسته‌ی صریحِ مالک: «هر دو» یک انتخابِ آگاهانه است، نه اثرِ جانبیِ یک دکمه).
   *  - `off` = «دیگه یادآوری نکن» ⟵ نیتش کلِ یادآوریِ شبانه است، پس **هر دو** خاموش
   *    می‌شوند. این نیمه دست‌نخورده ماند.
   *
   * قاعده‌ی عمومی (بند ۹ب/۶ ریشه): وقتی معنیِ یک ستون عوض می‌شود، همه‌ی **نویسنده‌هایش**
   * باید همان لحظه بازبینی شوند، نه فقط خواننده‌هایش. */
