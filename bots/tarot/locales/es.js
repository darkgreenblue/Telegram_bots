...
export default {
  ...
  buttons: {
    ...
    chatEnd: '🙏 Terminar la conversación',
    ...
    cardSwitch: '🔄 Cambiar número de tarjeta',
  ...
  // ☰ Etiquetas de los comandos del menú junto al campo de texto (v3.95.0). Muy cortas.
  commands: {
    menu: 'Menú principal',
    fal: 'Lectura nueva',
    support: 'Soporte',
  ...
  wallet: {
    ...
    cardSwitchHeader: '👇👇🔄 Nueva factura con un nuevo número de tarjeta 💳👇👇',
    // ⛔️ فازِ ۵ (v3.127.0): انتقال به کارتِ فاکتور ناموفق بود ⟵ همان فاکتور با کارتِ سفید.
    transferErrorHeader: '⛔️ Parece que la transferencia a esa tarjeta no se completó. Paga a la nueva tarjeta de abajo y envía el comprobante.\n\n👇👇🔄 Nueva factura con un nuevo número de tarjeta 💳👇👇',
    transferErrorApproved: '✅ Tu pago está confirmado, no hace falta pagar otra vez.',
    cardSwitchNone: 'No hay otra tarjeta disponible ahora. Si la transferencia falla, usa 💬 Soporte.',
    cardSwitchUsed: 'La tarjeta de esta factura ya se cambió una vez.',
    ...
    receiptNoInvoice: 'Esta foto no se vinculó a ninguna factura activa 🙏\n\nSi ya pagaste, abre «Diamantes», elige el paquete de nuevo y envía el comprobante ahí.\nSi algo falla, escribe a soporte y lo revisamos a mano.',
    receiptAlreadyDone: 'Este pago ya fue revisado 🙏\n\nSi crees que algo no cuadra, escribe a soporte y lo miramos a mano.',
    receiptAsFile: 'El comprobante llegó como **archivo** y no pude vincularlo a tu pedido 🙏\n\nEnvía la misma imagen otra vez, pero **como foto**.',
  ...
  errors: {
    ...
    openPaymentFlow: 'Tienes una compra a medio hacer 🧾 Primero elige tu paquete o cancélala con el botón de abajo.',
    voiceTooLong: (sec) => `No puedo recibir un audio de más de ${fmt(sec)} segundos 🙏 Manda uno más corto o escribe tu pregunta.`,
    packRetired: 'Este paquete ya no está disponible 🙏 Elige uno de los de abajo 👇',
  },
...
Devuelve solo un JSON válido, sin ninguna palabra de más y sin code fence:
{
  ...
  "closing": "empieza con «En general»: repite la respuesta, abre el «pero», **da un plazo aproximado** («en estas próximas semanas», «antes de que termine la temporada»), porque la persona quiere saber cuándo, y termina con una condición que está en su mano («si ..., entonces ...»). **La última frase es esa condición; después de ella no escribas nada más.** La condición tiene que decir en voz alta el nombre de una de las cartas de esta lectura y engancharse a su pregunta, no puede ser un consejo genérico. Una condición sin el nombre de una carta es consejo genérico. De 4 a 7 frases cortas",
  "summary": "cierre de máximo 300 caracteres, para el recuerdo",
...
Agrega al mismo JSON una clave más: "question_text" con el texto exacto de la pregunta que escuchaste (en español, máximo 300 caracteres, sin interpretación).`,
    ...
    chatFixHint: ({ thin = false, offer = false, safety = false, min = 0 } = {}) => [
      `⚠️ Responde la misma pregunta otra vez, de nuevo solo ese único JSON.`,
      thin ? `Tu respuesta anterior fue demasiado corta y vacía: esta vez aporta algo concreto y nuevo (abre una carta de esta lectura por su nombre o ánclate en lo que dijo), mínimo ${min} caracteres.` : '',
      offer ? `Tu respuesta anterior no tenía propuesta final: llena offer con una propuesta precisa y ligada a esta conversación, solo con la forma «¿Quieres que …?» o «Si quieres, puedo … por ti».` : '',
      safety ? `Solo se siente mal y no habló de hacerse daño: no menciones autolesión, suicidio, emergencias ni «¿estás a salvo?»; acompáñalo con calidez y sencillez.` : '',
      `Sin cortesías y sin repetir la pregunta.`,
    ].filter(Boolean).join('\n'),
