...
PRODUCT_VERSION = "1.7.1"   # 1.7.1: مرحله‌ی تصویر دیگر تحویلِ تعبیر را معلق نمی‌کند (سقفِ ۴۵ثانیه + تایم‌اوتِ هر زیرمرحله + لاگِ تفکیکی)
...
IMAGE_TIMEOUT     = 45    # سقفِ کلِ مرحله‌ی تصویر (ریکوئست + probe + احتمالِ ری‌تلاشِ سیاه)
IMAGE_REQUEST_TIMEOUT = 35   # سقفِ یک ریکوئست به سرویسِ تصویر (کلاینت خودش هم قطع می‌کند)
IMAGE_PROBE_TIMEOUT   = 10   # دانلودِ تصویر فقط برای ابعاد/چکِ سیاهی — کاملاً اختیاری
