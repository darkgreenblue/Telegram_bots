...
PRODUCT_VERSION = "1.7.3"   # 1.7.3: سؤالِ پرسونا صریح «اعتقادت چیه» می‌پرسد + ترتیبِ گزینه‌ها برعکس شد (عقلانی اول، مذهبی آخر)
# 1.7.2: تصویر روی CDNِ خارجیِ GapGPT با فالبکِ دامنه‌ی اصلی + سقفِ ۲۵ثانیه + کلیدِ خاموشیِ IMAGE_ENABLED
# 1.6.0: دکمه‌ی «💬 پشتیبانی» در منوی پایین (مشترکِ همه‌ی ربات‌ها) + /support — لینکِ چتِ پشتیبانی با پیامِ آماده‌ی حاویِ کدِ #DRM-<user_id> (support.py)
...
# دو endpointِ رسمی وجود دارد و مستنداتِ GapGPT صریح می‌گوید `api.gapapi.com` «CDN خارجی»
# است. سرورِ ما خارج از ایران است، پس CDNِ خارجی اول امتحان می‌شود و دامنه‌ی اصلی فالبک
# می‌ماند: هنگ‌های ۱۲۰ثانیه‌ایِ ۱۷ مرداد ۱۴۰۵ همگی روی endpointِ داخلی افتادند.
# هر دو با یک کلید کار می‌کنند. override با env بدونِ نیاز به کامیت: GAPGPT_BASE_URL.
GAPGPT_BASE_URL  = os.getenv("GAPGPT_BASE_URL", "https://api.gapapi.com/v1").strip()
GAPGPT_BASE_URL_FALLBACK = "https://api.gapgpt.app/v1"
IMAGE_MODEL      = "gapgpt/z-image"   # url مستقیم برمی‌گرداند — دست نزن

# کلیدِ خاموشیِ کلِ مرحله‌ی تصویر (رول‌بکِ یک‌خطیِ بند ۹ب ریشه). اگر سرویسِ تصویر
# پایدار خراب شد، این را False کن: تعبیر بدونِ هیچ انتظاری تحویل می‌شود.
IMAGE_ENABLED = True
...
# تصویر «تزئینِ» تعبیر است، نه خودِ محصول؛ پس هرگز نباید تحویلِ تعبیر را طولانی معلق کند.
# سقفِ ۱۲۰ ثانیه‌ی قبلی یعنی وقتی سرویسِ تصویر هنگ می‌کرد، کاربر دو دقیقه فقط جمله‌های
# «صبر کن» می‌دید و اغلب قبل از تحویل ربات را رها می‌کرد (باگِ ۱۷ مرداد ۱۴۰۵).
IMAGE_TIMEOUT     = 25    # سقفِ کلِ مرحله‌ی تصویر (هر دو endpoint + probe + ری‌تلاشِ سیاه)
IMAGE_REQUEST_TIMEOUT = 12   # سقفِ یک ریکوئست به یک endpointِ تصویر (دو endpoint داریم)
IMAGE_PROBE_TIMEOUT   = 8    # دانلودِ تصویر فقط برای ابعاد/چکِ سیاهی — کاملاً اختیاری
