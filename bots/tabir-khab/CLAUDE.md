...
- **آنالیتیکس:** `symbol_opened` (props: via=keyboard/paywall)، `symbol_viewed` (letter/word/persona)، `symbol_cta_dream`. قیف: symbol_opened → symbol_viewed → symbol_cta_dream → product_delivered.
- **فلوی اصلی تعریف خواب هیچ تغییری نکرده** — نمادیاب فقط نقطه‌ی ورود اضافه می‌کند.
