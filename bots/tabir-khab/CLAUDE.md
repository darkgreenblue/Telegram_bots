...
- **منوی اصلیِ اینلاین + بازگشتِ درختی:** `C.main_menu_message` (دکمه‌های `menu:new_dream|symbols|sub|persona`) هابِ ناوبری است. دکمه‌ی «بازگشت» (`C.back_row` → `menu:home`) زیرِ این حالت‌ها هست: خانه‌ی نمادیاب (→ منوی اصلی)، لیستِ کلماتِ نماد (→ گریدِ حروف، جایگزینِ «همه‌ی حروف»)، «خواب جدید»، وضعیتِ اشتراکِ فعال (+ دکمه‌های خواب جدید/نمادیاب)، «تغییر سبک». `_cb_menu` همه را **ادیت‌درجا** می‌کند (`_edit_or_send`؛ اگر ادیت نشد پیام جدید). لیستِ نماد به‌جای قبل/بعد، دکمه‌های شماره‌ی صفحه دارد و صفحه‌ی فعلی «✅» می‌گیرد.
- **استثنای آنبوردینگ (مهم برای نرخِ تبدیل):** کاربرِ تازه‌وارد تا قبل از رسیدن به دمو/پی‌وال، همان فلوی روانِ فعلی را می‌بیند؛ پیام‌های آنبوردینگ/قیف (welcome، سؤالات، دموی تریال، پی‌وال) **دکمه‌ی بازگشت ندارند**. منوی اصلی و بازگشت فقط از حالت‌هایی می‌آیند که کاربر با اقدامِ صریح (دکمه‌ی منوی پایین یا CTA) به آن‌ها می‌رود — که آنبوردینگ را طی نکرده‌ها به آن نمی‌رسند. متن‌های جدید: `main_menu`, `btn_back` (top-level هر ۶ locale) و `sym.page_btn`.
