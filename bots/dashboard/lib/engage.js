...
/* ── لایه‌ی دوم: «اکشنِ مفید» ──────────────────────────────────────────────
   ...
   ⚠️ دو قاعده‌ی ورود، هر دو از تصمیمِ صریحِ مالک (۱۴۰۵/۰۶/۱۵):

   ۱) **دنباله‌ی یک فلوی دیگر، اکشنِ مستقل نیست.** `feedback` (نمره‌دادن) عمداً
      این‌جا نیست: کاربر در انتهای فالِ پولی نمره می‌دهد، پس نمره تقریباً همیشه
      زیرمجموعه‌ی همان روزِ فال است و چیزی به سنجه اضافه نمی‌کند. و حالتِ نادری که
      اضافه می‌کند دقیقاً حالتِ **غلط** است: کسی که یادش رفته بوده و روزِ دیگری
      فقط برگشته نمره بدهد، یک روزِ فعالِ ساختگی می‌گرفت بدونِ اینکه هیچ ارزشی
      برداشته باشد. قاعده: اکشنی که فقط **دُمِ** یک فلوی دیگر است، روزِ فعال نمی‌سازد.

   ۲) **فیچرِ خاموش، سنجه نمی‌گیرد.** قلاب‌های منوی رایگان پشتِ `FREE_MENU_ENABLED`
      در خودِ ربات‌اند و آن فلگ **خاموش** است، پس رویدادهایشان ساختاراً هرگز شلیک
      نمی‌شوند. نامشان در این لیست نه عددی عوض می‌کرد و نه بی‌ضرر بود: به مالک
      وعده می‌داد سنجه چیزی را می‌پاید که کاربر اصلاً نمی‌بیند. با روشن‌شدنِ آن
      فلگ، همان لحظه یک خط این‌جا اضافه شود — نه زودتر. */
