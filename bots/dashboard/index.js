...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    /* ---- 🔀 بوکمارکِ کهنه‌ی صفحه‌ی حذف‌شده‌ی «هزینه‌ها» ----
       `/costs` در ۱۴۰۵/۰۶/۱۵ حذف شد چون یک صفحه‌ی درآمدِ **دوم** بود با انتخابگرِ
       بازه‌ی جدا و عددی که با صفحه‌ی اقتصاد نمی‌خواند (همان چیزی که مالک گزارش کرد).
       ولی مالک ماه‌ها از همان آدرس استفاده می‌کرد؛ ۴۰۴ دادن به بوکمارکش یعنی فکر کند
       داشبورد خراب شده. یک ریدایرکت به جایی که محتوا واقعاً رفته، صادق‌تر است. */
    if (path === '/costs') return redirect(res, `/economics${url.search || ''}`);
