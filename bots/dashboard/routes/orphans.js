...
export function orphanResolve(body) {
  ...
  /* ⚠️ `[0]` فقط وقتی معنی دارد که ربات **یک** دیتابیس داشته باشد. رباتِ چنددیتابیسی
     (tarot-intl: هر زبان یک فایل) با این خط الماس را به یک زبانِ **تصادفی** می‌داد و
     کاربرِ واقعی هیچ‌وقت نمی‌گرفتش — بی‌صدا، چون ردیف در داشبورد «حل‌شده» می‌شد.
     تا امروز فقط به این دلیل پیش نمی‌آمد که پرچمِ اعتباریِ tarot-intl خاموش بود، یعنی
     گاردِ واقعی یک **اتفاق** بود نه یک تصمیم. حالا که پرچم درست شد، این هم صریح شد.
     در عمل هم نباید پیش بیاید: پرداختِ سرگردان یعنی کارت‌به‌کارت، و زبان‌های دیگر روی
     ریلِ استارزند که اصلاً سرگردان نمی‌شود. پس امتناعِ صریح، نه حدس. */
  const insts = instancesOf(r.bot);
  if (insts.length > 1) {
    throw new Error(`«${botByKey(r.bot)?.title || r.bot}» چند دیتابیس دارد (${insts.map((i) => i.id).join('، ')}) `
      + 'و این ردیف نمی‌گوید مالِ کدام است؛ الماس می‌رفت به یک زبانِ تصادفی. '
      + 'اعتبارش را از صفحه‌ی «پشتیبانی» روی همان زبانِ درست بده و بعد این ردیف را ببند.');
  }
  const inst = insts[0];
