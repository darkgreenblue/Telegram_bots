...
هر ربات در فاز تست دکمه‌ی persistent با متن `🔄 ریست ربات (تست)` دارد (پیاده‌سازی مشترک: `shared/reset.js`؛ voice2text نسخه‌ی inline خودش را دارد). ربات بدون‌درآمد: برای همه فعال؛ ربات درآمدزا (voice2text): **فقط OWNER_ID** (کاربر پولی تصادفاً کیف‌پولش را پاک نکند). تابع wipe باید همه‌ی جدول‌های کاربرمحور همان ربات را پاک کند. پایان فاز تست: `TEST_PHASE=false` (دکمه مخفی؛ `/reset` برای مالک می‌ماند). **وضعیت فعلی:** voice2text و tarot هر دو زنده‌اند (دکمه برای کاربر عادی وجود ندارد؛ ریست فقط مالک)؛ فقط tabir-khab و ربات‌های آینده در فاز تست‌اند.
