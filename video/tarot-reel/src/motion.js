...
// باکس برای محاسبه‌ی گیت کمی «چاق» در نظر گرفته می‌شود. بدونِ این حاشیه، لحظه‌ی دقیقِ
// قفل‌شدنِ گیت همان لحظه‌ای است که کارت مماسِ باکس می‌شود و کمترین فاصله به چند پیکسل می‌رسد:
// امن، ولی بدونِ هیچ فضایی برای تیونِ آینده. با این حاشیه، کفِ فاصله همیشه یا حاشیه‌ی دالان
// (`CORRIDOR_PAD`) است یا همین عدد.
...
export function corridorGate(y, h, y0, y1) {
  const topClear = SAFE_BOX.y - h - SAFE_MARGIN; // بیشترین yای که هنوز از بالا امن است
  const botClear = SAFE_BOX.y + SAFE_BOX.h + SAFE_MARGIN; // کمترین yای که از پایین امن است
...
// حرکت **سه ضرب** است و ترتیبش عمدی: اول کارت به دالان می‌آید، بعد در دالان بالا/پایین
// می‌رود، و در آخر از دالان به جای خودش می‌رود و هم‌زمان بزرگ (یا قبلش کوچک) می‌شود.
//
// چرا سه ضرب و نه دو ضرب (باگِ دومِ دیده‌شده‌ی مالک: «بازم یه جاهاییش رباتی است»):
// گیتِ ایمنی وقتی کارت واردِ باندِ عمودیِ باکس می‌شود ۱ می‌شود، و چون فاصله‌ی ردیفِ بالا تا
// آن باند فقط ۱۶ پیکسل است، آن گیت عملاً یک **پله** بود. هر پله در گیت یعنی x در یک فریم
// به مرکزِ دالان پرت می‌شود: شتابِ ناگهانیِ اندازه‌گیری‌شده ۳۵۰ پیکسل بر فریمِ مربع.
// حالا x **قبل** از شروعِ سفرِ عمودی به دالان رسیده و **بعد** از تمام‌شدنش از آن بیرون
// می‌آید، پس در تمامِ مدتی که گیت ۱ است x از قبل همان‌جاست و آن پله دیگر هیچ چیزی را
// جابه‌جا نمی‌کند. گیت به‌عنوانِ تضمینِ ایمنی سرِ جایش است، فقط دیگر دیده نمی‌شود.
// پنجره‌ها عمداً **همپوشانی** دارند: اگر x دقیقاً همان‌جا تمام شود که y شروع می‌کند، کارت
// سرِ گوشه به سرعتِ صفر می‌رسد و می‌ایستد. با همپوشانی، گوشه گرد می‌شود و حرکت یک ژستِ
// پیوسته می‌ماند. اندازه‌ی همپوشانی محدود است به فاصله‌ای که کارت پیش از رسیدن به باندِ
// خطر دارد، وگرنه گیتِ ایمنی وسطِ راه x را پرت می‌کند.
const X_IN_GROW = [0, 0.26];
const Y_GROW = [0.2, 0.74];
const X_OUT_GROW = [0.66, 1];
const H_GROW = [0.68, 1];

const H_SHRINK = [0, 0.3];
const X_IN_SHRINK = [0, 0.3];
const Y_SHRINK = [0.26, 0.8];
const X_OUT_SHRINK = [0.72, 1];
...
export function corridorPath(from, to, side, t) {
  ...
  // ⚠️ تنها **یک** ایزینگ در کلِ مسیر هست و آن هم اینجاست. هر بخش (x، y، ارتفاع) بعد از
  // این، داخلِ پنجره‌ی خودش **خطی** حرکت می‌کند.
  // چرا: وقتی هر بخش ایزینگِ خودش را داشت، هر کدام از صفر شروع و به صفر ختم می‌شد، پس
  // پروفایلِ سرعت سه قله‌ی جدا با دره‌ی نزدیکِ صفر می‌شد؛ یعنی کارت سه بار راه می‌افتاد و
  // می‌ایستاد. دقیقاً همان چیزی که «رباتی» دیده می‌شود. حالا شتاب و کاهشِ سرعت یک بار و
  // برای کلِ ژست اتفاق می‌افتد و بخش‌ها بدونِ ایست دستِ هم را می‌گیرند.
  const u = easeInOut(tt);
  const seg = (w) => clamp01((u - w[0]) / Math.max(1e-6, w[1] - w[0]));
  const hRaw = lerp(from.h, to.h, seg(hWin));
  ...
  const xIn = grow ? X_IN_GROW : X_IN_SHRINK;
  const xOut = grow ? X_OUT_GROW : X_OUT_SHRINK;
  const p = seg(yWin);
  const yRaw = lerp(from.y, to.y, p);
  ...
  if (!crossesSafeBand(from, to)) {
    ...
    const w = cardW(hRaw);
    return { x: lerp(fromCx, toCx, seg([xIn[0], xOut[1]])) - w / 2, y: yRaw, w, h: hRaw };
  }

  const inner = corridorInner(side);
  // پنجره‌های x در **زمانِ کلِ مسیر** تعریف شده‌اند، نه در پیشرفتِ سفرِ عمودی: دقیقاً همین
  // است که ورود به دالان را جلوترِ از حرکتِ عمودی می‌اندازد.
  const inF = seg(xIn);
  const outF = seg(xOut);
  const cxRaw = lerp(lerp(fromCx, inner.cx, inF), toCx, outF);

  // گیتِ حس بینِ دو پنجره دقیقاً ۱ است، یعنی در تمامِ سفرِ عمودی کارت میخ‌کوبِ دالان می‌ماند.
  const gateT = Math.min(inF, 1 - outF);
...
// سهمِ پروازِ پایانی عمداً بزرگ‌ترین فاز است: مسیرش سه ضرب دارد (به دالان، بالا، به ردیف)
// و با سهمِ مساوی هر ضرب فقط شش فریم می‌گرفت، یعنی همان رباتی‌بودنی که مالک دید.
export const INTRO_PHASES = { fan: [0, 0.2], pull: [0.2, 0.4], flip: [0.4, 0.62], fly: [0.62, 1] };
// `TITLE_BOX` و `TOP_ROW` هر دو بالای باکس‌اند و روی هم می‌افتند، چون هر دو از تنها فضای
// بالای قاب استفاده می‌کنند. تصادم واقعی نیست چون هم‌زمان نیستند: عنوان باید **پیش از**
// رسیدنِ کارت‌ها به ردیفِ بالا محو شده باشد. این پنجره کمی زودتر از فازِ پرواز تمام می‌شود.
export const TITLE_FADE = [0.5, 0.62];
// دکِ بسته از پایینِ قاب بالا می‌آید؛ این مقدار «چقدر پایین‌تر» است.
// ورودِ دک از پایین عمداً کوتاه است: هرچه بیشتر پایین برود، بیشتر زیرِ نوارِ پایینِ
// اینستاگرام می‌رود و بیننده نیمی از حرکت را اصلاً نمی‌بیند.
const FAN_ENTER_DY = 40;
...
function introFly(i, n) {
  ...
  const span = b - a;
  // پلکانِ کوچک‌تر: با ۰.۴ هر کارت یک‌سومِ پنجره را از دست می‌داد و مسیرش تند می‌شد.
  const stagger = n > 1 ? (span * 0.22) / (n - 1) : 0;
